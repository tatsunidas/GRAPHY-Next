...
## 🔴 画面が小さくなったときに画面外を復元する（2026-08-13 実機で遭遇・未修正）
...
**推測**: サニタイズ／再クランプが、**`displayId` が一致したまま解像度だけ小さくなった場合に
効いていない**のではないか。ディスプレイの増減（displayId の変化）は見ているが、同じ
ディスプレイが低い解像度になった場合を拾えていない可能性。

**再現条件の見当**: 大きなモニタで使った後、小さい画面（ノート単体・解像度変更・リモート接続）で
起動する。ウィンドウが見えないため、利用者からは**アプリが起動しないように見える**。

**未修正**。復元時に必ず現在のワークエリアと突き合わせ、はみ出す場合はクランプする、
という形に直すのが素直だと思われる。
