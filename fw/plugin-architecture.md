...
| **H40** ✅ **実機 56/0** | **XA シネの時間軸** — fps とその出自・生タグ・各フレームの開始時刻 | `getXaCine(tileId?) => ViewerTileXaCine \| null` | 実装は `viewer/xaCineTiming.ts`（**純関数へ切り出した**・2026-09-06）＋ `viewer/xaCine.ts` の `readXaCineSource()`。🔴 **fps だけを返さない**——「タグから決まったのか既定値（15fps）に落ちたのか」が分からないと**フレーム番号を秒に換算してよいか判断できない**。本体は既定値に落ちたランで TIMI の 30fps 換算値を出さないと決めている（`fw/angio-design.md` §24.2）ので、プラグインだけが黙って埋めると製品内で規則が食い違う。🔴 **DSA 表示中も返す**——合成 imageId は元の URL を持たないので `dsaNativeImageId()` で委譲する（忘れると「差分にした瞬間に解析だけ静かに落ちる」）。🔑 経過時間は `frameStartTimesMs` の**差**で取る（**フレーム差 × 1/fps では可変レートで合わない**）。⚠️ dataSet が**プリウォーム前**なら null（ここで待つとプラグインの問い合わせがネットワーク取得を誘発する）。✅ **実機検証済み**（`automator/src/spike/qfrPluginCheck.ts`・2026-09-06）: 非 DSA（11 枚・FrameTime 33ms）と **DSA 表示中**（25 枚・66ms）の両方で `fpsSource: "frameTime"` を返し、`frameStartTimesMs` は `[0, 66, 132, …]`・末尾 1584 = 24×66。🔴 **`numberOfFrames` を必ず見ること** —— 単一フレームの静止画も `FrameTime` / `CineRate` を持つので、**この API はもっともらしい fps を返す**（実測: 3D 再構成用の静止画で fps 25・出自 frameTime）。フレーム数を見ずに「時間軸がある」と判断すると、フレームカウントが成立しないランで操作が止まる | ✅（standalone / web とも。XA / XRF のときだけ） |
| **H11** ✅ | **再構成済み 3D 血管モデルの取得** — 中心線・点ごとの内腔径・分岐トポロジ・校正・出自 | `listVesselModels()` / `getVesselModel(runId?)` | 実装は `plugins/pluginVesselApi.ts` ＋ `viewer/xaVesselModelStore.ts`（A6a / A6b の結果の登録簿・BroadcastChannel でウィンドウ間同期・**セッション限り**）。🔴 **測れなかった点の径は null**——0 や補間値で埋めると「そこが細い / 太い」という所見に化ける。🔴 **`diameterMethod` を必ず見る**（半値法と密度計測で径が 10% 以上違い、断面積はその 2 乗で効く。係数を定数として持ち回らない＝`fw/angio-design.md` §16.4 / §16.5）。**校正の縮退区分を運ぶ**ので、近似を実測として渡さない | ✅（standalone / web とも。XA の 3D 再構成があるときだけ） |
| **H12** ✅ | **解析結果（点ごとの値）の書き戻し** — 3D 血管に色マップと凡例で乗せる | `putVesselAnalysis(runId, result)` | 🔴 **本体は FFR を計算しない**（流体解析・学習モデルは外部。`fw/angio-design.md` §11.1 / §19。registration で DL を本体に載せないと決めたのと同じ線引き）。本体は**値の意味を解釈しない**——閾値も正常/異常の判定も持たず、`range[0]` を赤・`range[1]` を青に写すだけ（**高いほど悪い量は符号を反転して渡す**）。**値の無い点はグレーで補間しない**（埋めると解析していない区間が解析済みに見える）。**壊れた入力は黙って落とさずエラー**（未知の segmentId・範囲外の index・非有限の値・退化した range）。`disclaimer` はそのまま表示し、**幾何の縮退区分は host が凡例に必ず出す** | ✅ |
...
| **H42** ✅ **実機 15/0** | **メイン画面の host に窓と保存領域** | `openWindow(opts)` / `loadStore(patientKey)` / `saveStore(json, {patientKey, version})` / `deleteStore(patientKey)`（`mainscreen.menu`） | 実装は 2D ビューアの H30・H8 と同じもの（`mainscreen/MenuBar.tsx`）。メイン画面には「表示中の患者」が無いので **`patientKey` は型で必須**（省略すると別の患者の領域に書き得るため、推測しない）。起票は UVS の取り込み画面（段 4・2026-09-26）。検査は `automator/src/spike/mainScreenHostCheck.ts` | ✅（窓は両モード。保存領域は standalone） |
| **H43** ✅ | **開くダイアログ** | `file.pickFiles({title, multiple, filters}) => {ok, paths} \| {ok:false, canceled}` | Electron の `dialog.showOpenDialog`（`graphy:pick-files`）。**ファイルだけ**（フォルダは不可。`graphy:pick-import` は DICOM 取り込み用の題とフォルダ選択を持つので流用しない）。返すのは**絶対パス**で、JAR へ渡して読む想定。JAR は本体と同じ権限で動くので、この口が新しい権限を生むことはない。⚠ OS のダイアログは automator が押せないので、選ぶ操作は手で確かめる | デスクトップのみ（web は `desktop-only`） |
| **H44** ✅ | **患者の検索** | `db.searchPatients(query) => PluginPatient[]` | 既存の `GET /api/patients?q=`（DbAdmin）を読むだけ。`patientKey = patientId \|\| patientName`（2D ビューアの同一患者判定と同じ）。**読み取りのみ** | ✅ |
| **H45** ✅ | **バックエンド面のジョブ** — 進み具合と取り消し | `runBackendJob(payload, {onProgress, signal, pollMs}) => {ok, result} \| {ok:false, cancelled?, error?}` | backend: `POST /api/plugins/{id}/jobs`・`GET/DELETE /api/plugin-jobs/{jobId}`（`PluginJobService`・ワーカー 2 本・終了後 1 時間保持）。🔑 **SPI は変えない**——args に JDK の関数型 `__progress`（`BiConsumer<Double,String>`）と `__cancelled`（`BooleanSupplier`）を入れて同じ `run(Map)` を呼ぶ（SPI を JDK 標準型だけに保つ方針）。同期の `/run` では入らないので、JAR は「無ければ何もしない」で書く。要求本文の同名キーは本体が上書きする。web モード・JAR 無しは**投入の時点で** 501（非同期に失敗させると「始まったのに何も起きない」に見える）。デモモードでは `/run` と同じく塞ぐ | standalone のみ |
| **H46** ✅ | **DB を変えたことを知らせる** | `db.notifyChanged({studyUids?, patientId?})` | `emitDbChanged(..., {includeSelf: true})`。🔴 既存の `emitDbChanged` は**送信元のウィンドウには届かない**（BroadcastChannel の仕様）ので、メイン画面から呼ぶプラグインの一覧が更新されなかった。`includeSelf` のときだけ同じウィンドウにも配る（既存の呼び出しは挙動不変）。本体の書き込み API（H4b・H9 等）は自分で知らせるので、別経路で DB を変えたときだけ使う | ✅ |
| **H47** ✅ **実機 20/0** | **動画の諸元と指紋** | `video.probe(path) => PluginVideoProbe` | backend `POST /api/plugins/{id}/video/probe`（`plugin/video/VideoProbe`）。🔑 **ffprobe を使わない**（本体は ffmpeg しか同梱していない）: 諸元は `ffmpeg -i` の出力から読み、**フレーム数は `-map 0:v:0 -c copy -f null` で数え直す**（コンテナのヘッダの値や「尺×fps」は AVI で平気でずれる）。元ファイルの SHA-256（パス・大きさ・更新時刻で覚える）と、同じ動画が既に保管庫にあればその所在（`alreadyImported`）も返す | standalone のみ（web 501） |
| **H48** ✅ | **動画を DICOM として取り込む**（本体の確認ダイアログつき・ジョブ） | `video.requestImportConsent({items:[{path, patient, seriesDescription?}], modality?, frameValues?}) => {ok, consentToken} \| {ok:false, error, issues?}`（従来の `{patient, paths}` も可）→ `video.importAsDicom(req, {onProgress, signal})` | 🔑 **患者は動画ごとに違ってよい**（2026-09-26 ユーザ要望）。札は **path → 患者の署名・シリーズの説明** で縛る。🔑 **ダイアログの前に本体が患者の指定を確かめる**（`POST /api/plugins/{id}/video/validate`・副作用なし: 新しい患者の ID が既にある `patient-exists`〈既存患者の key・氏名つき〉/ 見つからない / 形の誤り / 同じ ID の食い違い）。問題があればダイアログを出さず `issues` を返す——**プラグインの重い処理（採点・変換）の前に止まる**（以前はジョブの中で初めて弾き、採点が終わってから失敗していた）。ジョブ内の検査は最後の砦として残す。🔴 **DICOM はプラグインに書かせない**（H4b / H9 と同じ）。変換（H.264 High・偶数寸法・`-bf 0`・faststart。包める MP4 は変換しない）・DICOM（`modality:"US"` なら **US Multi-frame**、既定は Video Photographic）・UID・患者属性・出所（`[Plugin] `・ContributingEquipment。**マニフェストから本体が入れる**）はすべて本体（`PluginVideoImportService`）。確認ダイアログ（`pluginVideoApi.ts`・DOM なのでメイン画面でもビューアでも出る）は**取り込み 1 回につき 1 回**で、返す札は**見せた患者・ファイル・書くもの（形式・SR）の範囲でしか使えない**（範囲外は backend に届く前に拒否。同じファイルは 1 札 1 回）。🔑 **重複の防止**: 動画の SOP / Series UID を元ファイルの SHA-256 から決める（`2.25.<名前ベース>`）ので、同じ動画は患者が違っても書かれず `duplicate` が返る。患者: 既存（`patientKey`）はその患者の既存インスタンスから Patient モジュールを写す／新規は**既にある患者 ID を拒否**（別人の記録に混ざるため。ただし同じ取り込みの 2 本目以降＝1 本目のスタディを渡したときは続けられる）。スタディは新規（2 本目以降は `studyInstanceUid` で同じ検査へ。**別の患者のスタディは拒否**）。`frameValues` を渡すと下の SR を同じ要求の中で書く。長さが動画の `NumberOfFrames` と違えば**動画だけ取り込み SR は書かない**（理由を `frameValuesError` に）。成功すると一覧の読み直しを知らせる（H46）。進み具合: 指紋 0〜10% → 変換 10〜90% → 書き込み。取り消しは ffmpeg を止める。**フレームごとの値の SR**（`FrameValuesSr`）: Comprehensive SR。根 CONTAINER「Per-frame values」→ 対象の動画（IMAGE）・出所・パラメータ（TEXT）・系列ごとの CONTAINER（フレーム順の NUM）。**値は丸めない**（DS に加えて FD にも入れる。DS は 16 文字で桁が落ち、閾値付近の比較が変わる）。私用スキーム `99GRAPHY-PLUGIN`（本体の計測概念 99GRAPHY とは分ける）。UID は動画とプラグインから決め打ち（採点し直すと置き換わる）。大きさの実測: 4,628 フレーム × 2 系列で約 2.5MB。検査は `automator/src/spike/pluginVideoImportCheck.ts`（奇数寸法の MJPEG AVI の変換・包める MP4・重複・長さ違い・既存/新規の患者・拒否・札の範囲外） | standalone のみ |
...
- 🔴 **画素統計と面積の出どころを本体の統計エンジンへ切り替えた（2026-08-27）。**
  正本は `fw/roi-stats-design.md` §7。**契約の型は変えていない**——意味だけを正した。

  | 項目 | 出どころ |
  |---|---|
  | `mean` / `stdDev` / `min` / `max` / `unit` / `area` | **本体の `viewer/roiStats.ts`** |
  | `length` / `shortAxis` | 従来どおり**ツール自身の計測**（Bidirectional はユーザーが引いた 2 軸そのもの） |
  | `longAxisMm` / `shortAxisMm` / `longAxisEnds` | 従来どおり `roiRead.computeCalipers()` |

  **なぜ切り替えたか**: Cornerstone の `cachedStats` は `voxelManager` から生のモダリティ値を
  読むので **SUV 校正を知らない**。契約は以前から `unit` を「"HU" / "SUVbw"」と書いていたのに、
  `"SUVbw"` は**構造上出せなかった**——PET を SUV 校正しても平均は Bq/mL のまま流れており、
  受け取ったプラグイン（線量計算など）は**例外も警告も出さずに違う値**を使う。
  本体の統計は `pixelCalibration` に一元化されているので、通せば HU も SUV も自動で正しくなる。

  **プラグイン側に起きる変化**（配布時のリリースノートに書くこと）:
  ① SUV 校正済み PET で `mean` などの**値が変わり**、`unit` が `"SUVbw"` になる。
  ② `area` が**メッシュ（閉多角形）の面積**になり、数 % 動き得る（ラスタ画素数 × 画素面積ではない）。
  ③ `unit` の空文字が減る（`RescaleType` が無いデータはモダリティ既定で埋める）。
  🔴 ただし **`"raw"` は「校正が無い」という意味**なので、定量に使う前に見ること。
  ④ 画素間隔が無いシリーズでは `area` が `undefined` になる（px² を mm² と偽らない）。

  🔴 **`cachedStats` へフォールバックしない。** `getRois()` は同期契約なので、統計が未計算なら
  キャッシュ済み画素での同期計算を試み、それも無理なら**すべて `undefined` を返す**。
  フォールバックを入れると、同じプラグインが**あるときは SUV・あるときは Bq/mL** を受け取り、
  どちらなのか誰にも分からなくなる。H35 の「未校正を数値で埋めない」と同じ考え方。
  表示中のスライスの画素は必ずキャッシュに載っているので、実用上ここで欠けることはまず無い。
...
#### H16 / H22 / H23 の実装（2026-08-23・線量評価プラグイン D4 からの要求）

> ⚠️ **当初は H25 も含んでいたが、main の H39 と重複していたため統合した**（2026-08-27）。
> 線量プラグインの「レポートへの差し込み」は **H39 経由で行う**（機能は失われていない）。

**動機**: 線量評価の結果を**病院システムへ渡す**経路が無かった。派生シリーズ（H4b）としては
保存できるが、受け側から見ると**ただの画像**で「線量」としては読まれない。VOI も同様に
「プラグインの中だけにあるマスク」で終わっていた。
...
- 🔴 **確認できない標準コードを書かない。** 吸収線量・時間積分放射能・有効半減期に対応する
  PS3.16 のコードは手元で確認できなかった。「それらしい標準コード」を書くと受け側は
  標準として解釈するので、**私用スキーム `99GRAPHY`**（＋ `CodingSchemeIdentificationSequence`）で
  書く。長径・短径の SRT コードはそのまま（実務で受け側が判別に使っている）。
  表は 1 か所（`SrMeasurementConcepts`）なので、**PS3.16 の該当コードが確認できたら 1 行差し替えで移行できる**。
- 🔴 **RTDOSE は「準拠していない点」を返す。** RT Dose モジュールの `ReferencedRTPlanSequence` は
  Type 1C だが、**放射性医薬品の線量評価に RT Plan は存在しない**。空の参照や作り話の UID を書くと
  受け側は「在るはずの計画」を辿って失敗する。**書かないかわりに `warnings` で返し、
  確認ダイアログでユーザーに見せる**（準拠していないことを、知らせずに作らない）。
- 🔴 **格子が一致しなければ書かない。** プラグインは H10 で読んだ格子の幾何を申告し、本体は
  元シリーズのスライス位置（`ipp + k·sliceStep`）と 0.1mm 以内で突き合わせる。
  **1 枚ずれた SEG は、見ないと気付けない。**
- **面法線と平行でない格子は RTDOSE では表せない**（`GridFrameOffsetVector` は法線方向しか持てない）。
  丸めて書くと受け側で面内にずれるので、**書けないものは書かない**で理由を返す。
- 実機検証で **SEG だけ出所が残らないこと**が分かった（他の 3 経路は `[Plugin] ` 接頭辞と
  `ContributingEquipmentSequence` を書いていた）。`SegExportRequest.producer` を足して揃えた。

**実機検証**（`automator/src/spike/exportApiCheck.ts`・**35/0**）: 真値既知のファントム GNBP-D で、
SEG が保管庫に入り**読み戻した前景ボクセル数が一致**すること・1 枚ずらした格子が拒否されること・
RTDOSE の**格納値 × `DoseGridScaling` が線量に戻る**こと・`GridFrameOffsetVector` の先頭が 0 であること・
RT Plan 欠落が警告として返ること・`NaN` があるのに背景未指定なら**同意を求める前に**拒否されること・
SR に私用コードで線量が入ること・`caveats` が空の登録が拒否されることを確認。

⚠ **automator の落とし穴を 2 つ踏んだ**（どちらも「検証が静かに嘘になる」種類）:

1. **孤児 Vite が 18093 を掴んでいた。** `--strictPort` は「他が居たら起動しない」だけなので、
   spawn は静かに失敗し、`waitForHttp` は**その孤児に**繋がって成功する。結果、
   **4 日前の別ワークツリーのフロントエンドで検証が走り**、新しく足した host API が
   「存在しない」と報告された。→ `desktopDriver` に `assertVitePortFree` を足した
   （backend 側には元からあった同じ守り）。
2. **`page.evaluate("window.__x = promise")` は固まる。** 代入式の値は promise なので
...
> 🔑 **H40 は QFR（`graphy-next-plugin-angio-quant`）のために足した**（2026-09-06）。起票は同 repo の `docs/host-api-gaps.md` の **G8**。造影流速は「3D 弧長 ÷ 通過時間」で決まるので、これが無いと**固定流速の fQFR しか出せない**（文献が精度 80%・陽性尤度比 4.8 で「臨床的に不十分」と明記した側）。
> 本体側の新規計算はゼロで、既存の `resolveXaFps()` / `frameStartTimesMs()` を**露出しただけ**。
>
> 🔑 **H35〜H39 が「なぜ生まれたか」の正本は `fw/angio-plugin-migration.md`**（2026-09-02 に
> 移設）。この表は結果であり、**無いと何が起きるか**はそちらに書いてある。外出しの試み自体は
> 段 2 で止まり、リポジトリ `graphy-next-plugin-angio-quant` は畳んだ。
