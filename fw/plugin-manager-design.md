...
## 5.5 取得の再試行（2026-08-24 追加）

実機で **GitHub のエッジが返した 1 回きりの 504** により導入が
「取得に失敗しました: HTTP 504」で終わった。直後に同じ URL を手で叩くと 200 が返り、
**完全に一過性**だった。ネットワーク越しの取得に再試行が無いのは実装の不足。

`HttpGitHubReleaseClient` は `listReleases` / `download` の両方で **最大 3 回**試す
（間隔 400ms → 1600ms）。再試行するのは待てば直るものだけ:

| ステータス | 扱い |
|---|---|
| **5xx / 429 / 408 / 接続エラー** | 再試行する |
| **404** | 即座に失敗。「リポジトリ名・リリースの公開状態・非公開ならトークン」を文言で示す |
| **403** | 即座に失敗。`X-RateLimit-Remaining: 0` なら「レート制限（未認証 60 回/時）。待つかトークンを設定」 |

🔴 **未認証だと 60 回/時**。同意画面の検査・版一覧・導入で複数回叩くので、
**試行錯誤していると普通に到達する**。到達時に「HTTP 403」とだけ出しても利用者は何も分からないので、
残量ヘッダを見て理由と対処を文言に入れている。
