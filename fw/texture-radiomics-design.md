...
1. ✅ **RadiomicsJ 2.4.0 は Maven Central に公開済み**（2026-08-12、`maven-metadata.xml` で確認）。
   このメモは公開前に書かれたもの。CI（3 OS）も 2.4.0 を解決して通っている。
2. 🔴 **実機検証が未実施** → **§11.10 で実施済み**（2026-08-13）。
3. **既存 Texture シリーズとの非互換**（margin 既定 3）。以前生成したマップとは値が比較できない。
   必要なら `margin=0` で再生成する。
4. ✅ **等方リサンプリングを実装した**（2026-08-13、§11.11）。設定 `texture.Resampling*` を消費する。
5. ✅ **GLAM の特徴量 150 個を表で出せるようにした**（2026-08-13、§12.10）。
   ただし出せるのは **`#glam` で解析した 1 ROI ぶん**で、複数症例を一括で回すバッチ実行は
   引き続きスコープ外（§9 の方針どおり。RadiomicsJ 本体の CLI／ImageJ プラグインが担当する）。

## 11.11 等方リサンプリング（2026-08-13）

**なぜ**: 特徴量の多くは距離をボクセル格子で測る。臨床 CT は面内 0.645mm・スライス 5mm といった
非等方が普通で（§11.10 の検証画像がまさにそれ）、そのままでは「隣」の意味が方向で 8 倍違う。
IBSI も抽出前の等方補間を求めている（IBSI 5.2）。距離ごとの構造そのものを見る GLAM では特に効く。

**実装**: `TextureResampling`（新規）。設定 `Resampling_BOOL` / `ResamplingX,Y,Z_DOUBLE`（mm）を読み、
`RadiomicsMapEngine.load()` の最後で画像とマスクを補間する。**マップ生成と GLAM 解析の両方**に効く
（両者は `load()` を共有している）。

- **補間そのものは RadiomicsJ に任せる**（`Utils.resample3D`）。IBSI 5.2.1 の格子中心合わせと、
  マスクの部分体積しきい値がすでに入っているため。
- 🚨 **設定は「利用者が触った項目しか保存されない」**。チェックだけ入れて X/Y/Z を触らないと
  backend には `Resampling_BOOL` しか届かず、登録簿（`settings/registry.ts`）の既定値 1mm は届かない。
  最初の実機確認で**いきなりこれを踏んだ**（「目標ボクセル間隔が設定されていません」で止まる）。
  → **未設定の軸は既定 1mm で補う**（`TextureResampling.DEFAULT_SPACING_MM` と
  frontend の `RESAMPLING_DEFAULT_MM` を同じ値に保つ）。明示的に 0 以下を入れた場合だけ断る。
  UI の注記も同じ既定で補うので、`resamplingSpacing()` に 1 本化してある。
- 🚨 **マスクには校正が入っていない**。`buildMask` が組み立てた `ImagePlus` は既定の 1mm 等方のままで、
  そのまま渡すと画像と違う格子へ落ちる。渡す前に画像と同じ `Calibration` を載せる。
- 🚨 **`Utils.resample3D(isMask=true)` はラベル 1 の二値しか受け付けない**（最大値が 1 を超えると例外）。
  `MASK_LABEL_INT` が 1 以外のときのために、1 へ寄せて補間し、戻してから返す。
- **可視化マップの幾何は変えない**。計算はリサンプリング格子で行い、出来上がったマップを
  **元シリーズの格子へ戻してから** DICOM 化する。派生シリーズの IPP / PixelSpacing / Rows×Cols が
  元と一致していることは Fusion で重ねる前提なので、ここは崩さない。
  - 逆変換の原点は `Utils.gridOrigin` を**呼んで**求める（自前で式を写さない）。順変換
    `x_src = origin + i_new / scale` の逆 `i_new = (x_src - origin) * scale` を使う。
    ここがずれるとマップは「それらしく」出たまま**元画像に対して平行移動する**ので、目視では気づけない。
    `TextureResamplingTest#mapsBackOntoTheSourceGridWithoutShifting` が線形勾配の往復で守っている。
- **増えすぎる指定は断る**: ボクセル数が 10 倍を超えるなら理由と実数を添えて拒否する
  （0.645×0.645×5.0 の CT を 0.2mm 等方にすると 200 倍を超え、GLAM だと時間単位になる）。
  1mm 等方は 512×512×50 → 331×331×250 の 2.6 倍で通る。
- **値の意味が変わる**ので `DerivationDescription` に `resampling=1x1x1mm` の形で残す。
  UI は Texture ダイアログと `#glam` の両方で、実行前に有効であることを出す。

**既定は OFF**。これまでに作ったマップと値が変わってしまうため、有効化は利用者の明示的な選択に委ねる。
...
## 12.9 グラフが「特徴量」ではなく「記述子」である件

よく聞かれるので明記しておく。**この画面のグラフは特徴量から作られていない。特徴量の方が、
このグラフを潰した結果**である。

```
① 動径分布関数 g(α,β,r)          ROI のボクセル対から 1 回だけ計算
        ↓ 曲線ごとに要約
② 親和性行列 19 種（nBins×nBins） 行列そのものが記述子（GLCM の共起行列に相当）
        ↓ 行列を 1 つの数値に潰す（GLAMFeatures#reduce）
③ 特徴量 150 個                   19 行列 × 8 統計（Compressibility は 6）
```

1 つの特徴は必ず **(行列, 統計) の組**として定義されている
（`SecondVirialCoefficient_Mean(GLAMMatrixType.SecondVirialCoefficient, GLAMStatistic.Mean)`）。
可視化マップが載せているのは ③。

| 図 | 段 | 特徴量との関係 |
|---|---|---|
| 自己親和性 g(α,α,r) | ① | 全 150 特徴の**共通の源**。どの特徴でもない |
| 親和性行列ヒートマップ | ② | この 1 枚が **8 個の特徴の入力**（＝潰す前の姿） |
| ビン占有数 | ①より前 | 特徴ではない。離散化の結果 |

**潰した瞬間に消える情報がある**ので、②を見せる価値がある。原著ノートの例では、同心球ファントムの
第二ビリアル係数の対角が濃度値 0 で **−5706.7**、濃度値 3 で **+1848.4** となり、
`SecondVirialCoefficient_DiagonalMean` を取ると**打ち消し合って情報が消える**。
特徴量 1 個だけ見ていると「何も無い」と読める構造が、ヒートマップなら一目で分かる。

### 非対角 g(α,β,r) も返すようにした（2026-08-13）

RDF は **nBins × nBins 本の曲線**（α と β の全組み合わせ）を持つ。当初は原著の図に合わせて
**対角 g(α,α,r) だけ**を返していたが、非対角＝異なる濃度値どうしの関係は**別の情報**であり、
親和性行列の非対角成分（＝8 統計のうち `Mean` / `OffDiagonalMean` などが見ているもの）の中身そのものなので、
返して描くようにした。

- backend `GlamAnalysis.crossAffinity` = `[α][β][r]`（対角も含む完全な形）。`rdf[r][α][β]` の並べ替えだけで、
  **計算は増えない**（元から全部計算されている）。
- **サイズだけが論点**。要素数は nBins² × maxRadius で、既定の 16 ビン・r=30 なら 7,680 個だが、
  64 ビン・r=50 だと 204,800 個（JSON で数 MB）になる。**20 万個を超えたら返さず、理由を添える**
  （`crossAffinityOmitted`）。保存すると `glam_analysis.json` にもそのまま入るため。
- 画面は「基準の濃度値 α」を選ばせ、その行の β 全部を重ねて描く。**α 自身の曲線＝自己親和性**なので
  太線で重ね、比較の基準にする。α の既定は**一番ボクセルが多いビン**（ビン 0 は空のことが多く、
  空だと曲線が 1 本も出ずに壊れて見える）。

## 12.11 マルチセグメント SEG のチャンネル選択（2026-08-13）

GLAM 解析はマスクのチャンネルを **0 固定**で送っていたため、DICOM SEG に複数セグメントが
入っていても**常に先頭のセグメントしか解析できなかった**（可視化マップ側には元から選択があった）。
実機で 4 セグメント（Liver / Mass / Portal vein / Abdominal aorta）の SEG を使おうとして判明。

- マスクを選ぶと `SeriesLayout.nC` を見て、2 以上のときだけチャンネル選択を出す（Texture ダイアログと同じ形）。
- **番号ではなく名前で選ばせる**。「0/1/2/3」だけでは、肝臓と腫瘍のどちらを解析した結果なのかが
  後から分からない。名前が取れなければ番号にフォールバックする。
- 🚨 名前のために `GET /api/dicom/seg` を呼んではいけない。**1 セグメントあたり
  512×512×スライス数の Base64** が返る（この SEG で実測 **28MB**）。
  マスク平面を組み立てない `GET /api/dicom/seg/segments` を足した（同じ SEG で **384 バイト**）。
- **チャンネル i ＝ セグメント番号の昇順 rank**（`SegFrameExpander` が C をそう決めている）。
  対応が崩れると「Liver を選んだのに Mass が解析される」になるので、
  `SegReadService` は<b>セグメントを番号昇順に整列してから返す</b>（読込側の並び順に依存しない）。

## 12.10 特徴量 150 個の表（2026-08-13）

記述子（①②）に対する「答え」の側（③）も同じ画面に出す。**19 行 × 8 列の格子**で、
150 行の一覧にはしない — 1 つの特徴は必ず (行列, 統計) の組として定義されているので、
同じ行列の 8 個が横に並ぶ形が読み方に合う（`Mean` と `DiagonalMean` で符号が違う、が一目で分かる）。

- backend は行列を全部組み上げた後に `GLAMFeatures#calculate(name)` を 150 回呼ぶだけ。
  行列はすでにあるので**統計で潰すコストしか増えない**。1 つ転んでも表全体を落とさず `null` を入れる。
- 空欄＝定義されない組み合わせ（自己ペアだけの行列 `Compressibility` は対角/非対角の統計を持たない、
  ROI にその濃度値が無い、など）。
- CSV（`name,value` の 150 行）で保存できる。**複数症例をまとめるのは受け取った側の仕事**という線引きは
  §9 のまま変えていない（バッチ実行は RadiomicsJ 本体の CLI／ImageJ プラグインが担当する）。
