...
- **自動振り分けはまだ OFF**（`mobile/mobileRoute.ts` の `MOBILE_SHELL_READY = false`）。
  確認するには **System メニュー →「モバイル UI に切り替え」** で手動で入る。
...
1. `mobile/mobileRoute.ts` の **`MOBILE_SHELL_READY` を `true` に**する（1 行）。
   これで web モードのスマホ利用者が自動でモバイル UI へ入るようになる。
   ⚠️ **公開デモの利用者も対象になる**ので、確認前に有効化しないこと。
