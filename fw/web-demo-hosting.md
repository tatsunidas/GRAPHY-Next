...
メモリモニタ・IJ/RTエクスポート）。RTSTRUCTのインポート（GET、閲覧系）とレポート機能は
意図的に表示のまま。`tsc -b`・ビルド成功、実機デプロイ済み（ブラウザでの目視確認は未実施）。
SEGエクスポートは 2026-07-16 にデモ制限から除外し表示するように変更（上記ステータス参照）。
...
**`graphy.vis-ionary.com/demo` の文言更新（2026-07-14）**: `src/pages/demo/index.astro`
（サイト移管により現在は `vis-ionary-web` リポジトリの `graphy-site/` 配下。旧 `website/`）
を「近日公開」から「メンテナンス中」の文言に変更した（実リンクへの差し替えはまだしない方針。
「まずダウンロードして試す」ボタンは維持）。

**SEG書き出しをデモ制限から除外（2026-07-16）**: `POST /api/dicom/seg` は毎晩の自動リストア
（`reset-demo.sh`）で確実に消えるため、他の「持ち出し」系（RTSTRUCT書き出し等）と異なり
デモ体験として許可することにした。`DemoModeFilter` のブロックリストから除外し、frontend
（`RoiManagerPanel.tsx` の「SEG ⬇」ボタン）の `!isDemo` ガードも外した。RTSTRUCT書き出しは
（PACSへ新規シリーズを恒久的に書き戻す想定の運用が異なるため）引き続きブロック。
...
`POST /api/dicom/rtstruct`、`/api/series/**`（派生シリーズ書き戻し）、
...
`/api/dicom/rtstruct` の GET 等）は許可のまま。`POST /api/dicom/seg`（SEG書き出し）も
毎晩の自動リストアで対応する前提で意図的に許可（2026-07-16、上記ステータス参照）。
...
- [x] `graphy.vis-ionary.com/demo`（Astro, 現 `vis-ionary-web` の `graphy-site/src/pages/demo/index.astro`、旧 `website/`）の文言更新
