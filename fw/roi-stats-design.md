...
### 4.9 🚨 キャッシュした表示内容は、描画時点の実体と食い違う（実機で踏んだ・2026-08-27）

**症状**: フリーハンド ROI を描くと**毎回落ちる**。

```
Uncaught TypeError: Cannot read properties of undefined (reading '1')
    at getTextBoxCoordsCanvas
    at FreehandRoiTool._renderStats
```

**原因**: 上流の `PlanarFreehandROITool._renderStats` は「`getTextLines` が空でない」ときだけ
textBox の位置計算へ進み、そこで **`data.contour.polyline` の先頭 2 点**を使う
（`getTextBoxCoordsCanvas` → `_determineCorners` が `canvasPoints[0]` / `[1]` を読む）。
輪郭を描いている最中の polyline は**空や 1 点になる瞬間がある**。

> 🔴 **上流の既定実装は「統計がまだ無いので空を返す」ことで、偶然この崩壊を避けていた。**
> §4.6 でキャッシュから返すようにした結果、その**暗黙の前提が消えた**。
> `data` の中身は描画のたびに変わるのに、キャッシュは前の内容を返し続ける。

**対策**: `graphyGetTextLines` は**描画時点の頂点数を必ず見る**（`canPlaceTextBox()`）。
面型・線型は 2 点未満なら行を返さない。プローブは別経路（`drawTextBox` をハンドル位置に描く）
なので 1 点で足りるため、種別で分ける。統計エンジン側も 2 点未満では何も測らない
（`周囲長: 0 mm` のような無意味な値を作らない）。

**一般化して覚えること**:
**上流が「値がまだ無い」ことで暗黙に守っていた前提を、キャッシュで置き換えると壊れる。**
描画のたびに変わる状態（`annotation.data`）をキーにしたキャッシュを返すときは、
**返す直前にその状態を検算する**。

他ツールも数え上げた: 矩形・楕円・長さは常に固定数のハンドルを持つので安全。
ポリゴンライン（自前の `_renderStats`）は元から 2 点未満をガードしていた。
...
| ✅ **RS5** | **H5 切替**（§7）＋ `fw/plugin-architecture.md` / テンプレート `.d.ts` 更新 | 済（別ブランチ `feat/roi-stats-h5`）。実機 36/0。**SUV での確認だけは fixture 未取得で未実施**（§10.1） |
| **RS6**（FutureWork） | テクスチャ（§11） | — |

各フェーズで `/verify`。`fw/` を更新。
**`tsc` / `vitest` 1109 件 / `build` すべて green。実機検証も実施済み（§10.1・36/0）。**
...
### 10.1 実機検証（automator）— ✅ 実施済み（2026-08-27）

ドライバ `automator/src/spike/roiStatsCheck.ts`。**教訓 2 つを踏んである**:

1. **「画面と同じか」を画面から測る**（`fw/angio-design.md` の 2026-08-23 の教訓。
   出力ファイルだけ見る検査では 1 件も掴めなかった）。
   → ROI 脇の textBox（Cornerstone の SVG）と、ダイアログの表を**どちらも DOM から読んで**
   突き合わせる。内部のストアを覗くと、表示されていない値でも合格してしまう。
2. **合成ローダを足したら「タグを直接読む層」を数え上げる** → ⚠ 未実施（下記）。

**結果: `ct-basic`（実 CT・50 スライス）で 36 件 ok / 0 件 FAIL。**

| 確かめたこと | 結果 |
|---|---|
| 7 ツールで ROI 脇に統計が出る（矩形・楕円・フリーハンド・ポリゴン・**ポリゴンライン**・**フリーライン**・長さ） | ✅ **開いた ROI でも長さ＋平均が出る**＝本件の核心 |
| 平均・面積が ROI 脇とダイアログで一致 | ✅ `-977.07 HU` / `1084 mm²` |
| 単位が実データで解決される | ✅ `HU`（`resolveValueUnit`） |
| 面積が mm²（校正済みシリーズ） | ✅ |
| 表示モード off / corner / beside | ✅ 右下一覧に `#1` とバッジが出る |
| CSV のヘッダに単位が入る | ✅ `mean[HU]`、ヘッダ＋1 行 |
| H5 がプラグインへ渡す値が画面と同じ（RS5） | ✅ `-977.0701754385965`（画面の丸め幅で一致） |

🔴 **切り替え前後の実測（§12.2-2 の「数字が動く」の答え）**:
同じ矩形 ROI に対し、**切り替え前（Cornerstone の `cachedStats`）は `-976.8496 HU`、
切り替え後（本体の統計エンジン）は `-977.0702 HU`**。差は **0.22 HU**。
面積は `1084.4624 mm²` 対 `1084.4626 mm²` で実質同じ。サンプル画素は 2622。
画素の拾い方（Cornerstone は矩形の索引範囲、こちらはメッシュの偶奇則）の違いで、
**境界の画素 1 個ぶんに相当する**。値が動くのはこの程度で、桁が変わるような話ではない。

⚠ **未検証（fixture 未取得）**:
- **SUV 校正済み PET**（`pet-suv`）＝ RS5 の目玉。「SUV 校正で値と単位が変わる」は未確認。
- **幾何の無い XA**（`xa-no-geometry`）＝ px 表示のまま mm を捏造しない経路。
- **DSA 合成中 / ThickSlab 中**の単位（§4.4）。

### 10.2 人手での実機確認（2026-08-27・all green）

automator とは別に、**利用者が実機で触って確認**した（`make dev-desktop`・マージ済み `main`）。

🔴 **automator が拾えなかった不具合が 1 件出た**——**フリーハンド ROI を描くと毎回落ちる**（§4.9）。
automator は `dragOnCanvasHost` が**一気に点列を送る**ので、polyline が 1 点しか無い
中間フレームが**描画されないまま過ぎていた**。人が手で描くと必ずその状態を通る。
**「automator が緑でも、人が触ると出る不具合がある」の実例**なので、
新しい描画系を足したら**人手で一度は描くこと**。

直したあと再確認して all green。

⚠ **検証の途中で踏んだこと（次に同じことをする人へ）**:
- **ブランチを取り違えて「不一致」を 3 回再現した。** RS5 は別ブランチにあるのに、
  ベースブランチのまま実機を回して「H5 の値が画面と違う」と読んでいた
  ——実際にはそれが**切り替え前の正常な姿**（Cornerstone の値）だった。
  **実機を回す前に `git branch --show-current` を見ること。**
- **full precision の値を、画面の丸めた表示と直接比べない。** 正しくても必ず落ちる。
  画面の丸め幅（小数 2 桁）に合わせて突き合わせる。
- **実行中にフロントを編集しない**（Vite の HMR が走って結果が汚れる）。
  `automator-verification-hygiene` の「編集と長時間実行を混ぜない」そのもの。
...
- 2026-08-27 **実機検証を実施**（`automator/src/spike/roiStatsCheck.ts`・ct-basic・36/0）。§10.1 に結果と
  切り替え前後の実測（0.22 HU）を記録。
- 2026-08-27 **人手の実機確認で 1 件出た**（フリーハンド ROI を描くと落ちる・PR #152）。
  §4.9 に原因と一般化した教訓、§10.2 に「automator が拾えなかった理由」を記録。
...
- 2026-09-10 **プローブに画素座標を併記**（利用者要望）。「信号強度だけでは、どの画素を読んだのかが
  分からない」——ImageJ など他のツールと突き合わせるときに致命的。
  - 🔴 **表示する座標は `centroidPx` を丸めたものではない。** 読み出し（`nearestValue`）は
    画像の外を端へ**クランプ**するので、画像外に置いたプローブでは「丸めた重心」と
    「実際に読んだ画素」がずれる。**表示座標とその画素値が食い違う**のは、
    この機能で唯一やってはいけないこと。
  - そこで `roiStats.nearestIndex()`（`nearestValue` が内部で使う同じ関数）を切り出し、
    読んだ画素インデックスを `RoiGeometryStats.samplePx` に載せて表示側へ渡す。
    整形は `roiStatsText.formatPixelCoord()`（純関数）。ROI 脇・隅一覧・計測結果ダイアログの
    3 経路とも同じ整形を通る。
  - `samplePx` が無ければ**行ごと落とす**（"—" を並べない＝既存方針どおり）。
  - 単位は px（col, row）。患者座標 mm は出さない（利用者の指定）。
  - ✅ **実機確認済み**（`automator/src/spike/roiImprovementsCheck.ts`）。プローブ脇の
    `座標: (426, 283)` / `67 HU` が、**状態バーの独立表示 `XY 425.6, 283.4` / `値 67 HU` と一致**した
    ——別経路の 2 つが合うことが「表示座標とその画素値が食い違っていない」の証拠になる。
