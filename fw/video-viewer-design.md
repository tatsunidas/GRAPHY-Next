...
### 4.4 変換結果のキャッシュ（**P1 実装済**）
- 抽出/変換した MP4 を **`<storageDir>/.video-cache/{sop}.mp4`** に保存（既存 `.import` 一時領域と同じ
  storage-dir 配下サブディレクトリ作法に倣う。当初案の `<appData>/cache/video` は storage-dir 起点に変更）。
  2 回目以降はキャッシュ（元 DICOM の mtime より新しければ）を返す。
- 無変換ケースはフラグメントが即 MP4 なので、初回だけ一時展開（temp→`Files.move` REPLACE）→ キャッシュ。
- 実装: `VideoRenderService.getOrExtract`（`synchronized`）。抽出は `VideoFragmentExtractor.extractTo`
  （`IncludeBulkData.YES` で各フラグメントを byte[] 連結。単一フラグメント＝MP4 全体の取込済みデータに最適。
  巨大動画のメモリ最適化=フラグメント逐次ストリーミングは将来課題）。
- キャッシュ無効化はインスタンス削除フックに合わせる（当面は mtime 比較で「古ければ作り直し」）。
...
- **P1（配信＋最小再生）… backend/frontend 実装済（実機検証は要フィクスチャ）**:
  1. ✅ backend `VideoRenderController`（`/rendered` Range 配信＋`/video-metadata`）。無変換経路のみ（取込済み H.264）。
     `VideoFragmentExtractor`/`VideoRenderService` ＋ テスト13（全170 green）。
  2. ✅ frontend **独立 `VideoViewer.tsx`（VideoViewport）**：`videoMetadataProvider.ts`（imageUrlModule/
     generalSeriesModule/cineModule/imagePlaneModule を供給）＋ 自前 RenderingEngine ＋ 再生 UI（再生/一時停止・
     フレームスライダ・前後フレーム・速度・クリップ選択）＋ `StudyList` の InstanceList からのルーティング
     （SeriesViewer 非経由）。`api.ts` に `videoRenderedUrl`/`fetchVideoMetadata`/`isVideoSopClass`。tsc green。
  - 残: 実機 E2E（取込済み H.264 動画 DICOM フィクスチャで再生/シーク確認）。§10-4。
