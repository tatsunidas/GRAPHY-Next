...
3. 🚨 **2026-09-25 訂正: 画面の表示は 1 フレームずれていた。** 下の「対処不要」は**解析用の別 `<video>`**
   （フレーム中心へシーク）について正しかったが、**画面の VideoViewport** は `setFrameNumber(n)` で
   **フレームの境目** `(n-1)/fps` へシークしており、ブラウザが描く絵が 1 つ前になることがあった
   （番号は進むのに絵が進まない＝ユーザ報告「本当に 1 フレームずつ進んでいるのか分からない」）。
   → `videoFrameTime.ts` の `frameToSeekTime`（フレームの 1/4 の位置 `(n-1+0.25)/fps`。中央にしないのは
   Cornerstone の読み戻し `1+round(t·fps)` が 0.5 を切り上げるため）に変えた。`videoFrameAccuracyCheck` に
   **描画面の画素そのもの**を読む検査を足し、境目へ戻すと落ちることも確かめた。
   （以下は 2026-07-30 当時の判断）✅ **フレーム精度シーク（2026-07-30 決着: 対処不要）**: 実機で測ると**キーフレームが先頭だけの動画でもフレームは
