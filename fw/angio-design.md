...
>
> **実機検証の到達点（合計 409 項目）**: A1 シネ 31 ／ A2 DSA 18 ／ A3 校正 11 ／
> A4+A10 QCA 20 ／ A4 手修正 34 ／ A9 RDSR 18 ／ **A4b ファントム 67（退行 0・設計目標に未達 17）**
> ／ **A13-1 ステップ・レール 30** ／ **A5b QLV 44** ／ **A6a 3D QCA 102（未達 3）** ／
...
>
> **残件（次のセッションはここから）**
> 1. **A6a の残り: 病変長の当てはめの見直し**（§10.2.8）。参照径が散布の**上包絡**へ寄るため、
>    2D より散る 3D のプロファイルでは病変長が過大に出る（実測 33.7mm / 真値 15.8mm）。
>    定義を 2D QCA と**共有している**ので、直すと A4b の実機 67 項目に影響する。**設計判断が要る**。
>    それ以外（3D 長さ・短縮・ワーキングアングル・断面合成・%DS・SR 保存・3D 表示）は
>    **実装＋実機検証済み**
> 2. **QCA のエッジ検出の作り直し**（§16.4）。円柱モデルの当てはめにすれば径の絶対値が
>    直るが、実血管は円柱ではないのでモデル化誤差と交換になる。**設計判断が要る**
> 3. **A13-3（独立画面）は急がない**。⚠️ タスクが 3 つ以上実装されてダイアログが実際に
...
> **A6a（3D QCA 単一血管）は前倒しで実装済み**（実機 102/0・§10）。残っているのは
> 病変長の当てはめだけで、これは **2D と共有する参照径の定義**に手を入れる話なので
> A5a（QVA＝参照径モデルの差し替え）と**一緒に決めるほうが早い**。
