...
>
> 🆕 **A6a に着手した（2026-08-15）**: **GNBP-XA-3 を生成**（4 視点＋角度誤差版）し、
> **射影幾何とバンドル調整を実装**（`viewer/xaGeometry.ts`・vitest 27 件・§10.1/§10.2）。
> 真値の中心線が画像の血管に乗ることも画素で確認済み（`bench/check_xa3_geometry.py` 16/16）。
> **残るのは中心線の対応付け・3D 表示・UI**（下の残件 1）。
>
> **残件（次のセッションはここから）**
> 1. **A6a の続き** … 2 方向の中心線を対応付けて 3D 中心線を作り、既存 3D ビューアへ載せる。
>    幾何とバンドル調整の土台はできている
...
| 3D 中心線 | RMS < 0.5〜1.0 mm | ⚪ **ファントムは生成済**。中心線抽出と対応付け（A6a の残り）が未実装のため未測定 |
| 3D 区間長（foreshortening 補正の実利） | 誤差 **< 3%**、かつ **2D 投影長より真値に近いこと** | ⚪ 同上。**「3D のほうが良い」を数値で示すのがこの行の役目** |
| 装置角度の回収（バンドル調整） | 注入した誤差との差 **< 1°** | ✅ **合成対応点で達成**（vitest）。実画像からの対応点では未測定 |
...
| **A6a** | 2D→3D 再構成・**3D QCA 単一血管**（射影幾何・バンドル調整・3D 表示） | A3, A4 | **大** | 🟡 **射影幾何とバンドル調整は実装済**（`xaGeometry.ts`・vitest 27）／ファントム GNBP-XA-3 生成済。**中心線の対応付け・3D 表示・UI が未** |
