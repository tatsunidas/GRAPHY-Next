...
> 起票: 2026-08-14 ／ ステータス: **A1・A2・A3・A4（手修正含む）・A5b・A9・A10・A13-1 実装済み・
> 実機検証済み（ブランチ `feat/angio-a1`）／ A5a・A5c・A6〜A8・A11・A12・A14 は未着手**
>
> **実機検証の到達点（合計 273 項目）**: A1 シネ 31 ／ A2 DSA 18 ／ A3 校正 11 ／
> A4+A10 QCA 20 ／ A4 手修正 34 ／ A9 RDSR 18 ／ **A4b ファントム 67（退行 0・設計目標に未達 17）**
> ／ **A13-1 ステップ・レール 30** ／ **A5b QLV 44**。スパイクは `automator/src/spike/xa*Check.ts`。
>
...
>
> 🆕 **A6a の再構成コアまで実装した（2026-08-15）**: **GNBP-XA-3 を生成**（4 視点＋角度誤差版）、
> **射影幾何とバンドル調整**（`viewer/xaGeometry.ts`・vitest 27 件・§10.1）、
> **中心線の対応付けと 3D 再構成**（`viewer/xaRecon3d.ts`・vitest 27 件・§10.2）。
> 真値の中心線が画像の血管に乗ることも画素で確認済み（`bench/check_xa3_geometry.py` 16/16）。
> **精度は設計目標を達成**（GNBP-XA-3 主枝で **RMS 0.089mm**・長さ誤差 0.9%・§10.3）。
> **残るのは 3D 表示と UI**（下の残件 1）。
>
...
>
> **残件（次のセッションはここから）**
> 1. **A6a の続き** … 3D 中心線を既存 3D ビューア（`viewer3d/`）へ載せ、UI（2 方向の選択・
>    アンカー指定・品質表示）を作る。**幾何・バンドル調整・対応付け・再構成の土台はできている**
> 2. **A14（レポート統合）** … QCA・QLV が実装済みなのに結果がレポートへ流れない（§21.5）
...
| 3D 区間長（foreshortening 補正の実利） | 誤差 **< 3%**、かつ **2D 投影長より真値に近いこと** | 🟡 **長さ誤差 0.9% は達成**。**2D 投影長との比較はまだ**（この行の本題はそちら） |
...
| **A6a** | 2D→3D 再構成・**3D QCA 単一血管**（射影幾何・バンドル調整・3D 表示） | A3, A4 | **大** | 🟡 **再構成コアまで実装済**（`xaGeometry.ts` vitest 27 ＋ `xaRecon3d.ts` vitest 27）。**GNBP-XA-3 で設計目標を達成**（RMS 0.089mm・§10.3）。**3D 表示と UI が未** |
