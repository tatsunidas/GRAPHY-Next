...
> GNBP-XA-6 のまま。🔴 **PNG を XA DICOM に包むラッパが未実装**なので、4 件はまだ取り込めない。
>
...
🔴 **PNG を XA マルチフレーム DICOM に包む変換器は、まだ無い。** §16.1-2 が
「`bench/` の生成器で XA DICOM に包んでから取り込む」と書いているのは**方針だけ**で、
`bench/` にあるのは `make_phantom_xa.py`（ファントム生成）と `dicom_io.py` だけである。
**上の商用利用可 4 件はすべて PNG / 画像なので、どれを使うにも先にラッパが要る**
（CADICA の 10 連続フレームをマルチフレームにまとめる処理も含む）。
