...
5. 🚨 **先に非円形のファントムを足す**（A4b の拡張）。楕円・偏心・三日月の断面を持つ系列が
   無い限り、**M3 の「形を仮定しない」利点は測定で示せていない**（現在のファントムは
   M3 の仮定そのもので作られている）。**これを A4c の着手条件にする。**
...
| **A4b** | GNBP-XA ファントムと精度検証ハーネス | A4 と並行 | 中 | ✅ XA-1/2/4 生成済・**実機 67 項目**（§16.4）。DSA ハーネスは未。**非円形断面の系列が未**（A4c の着手条件・§16.5） |
| **A4c** | **エッジ検出の作り直し**（報告する径を密度計測へ。輪郭は半値法のまま） | A4b の非円形系列 | 中 | 🔴 **方式は決定済み（§16.5・対照実験で決着）**。着手前に非円形ファントムを足すこと |
