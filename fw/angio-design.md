...
| 仮番 | 内容 | 無いと |
| :-: | :- | :- |
| **G1** | 空間校正の**出自・信頼度・警告**を読む | mm/px は取れるが「近似か実測か未校正か」が分からず、§7.4 が要求する注記を**プラグインが書けない** |
| **G2** | XA の状態（DSA のマスク・シフト・`PixelIntensityRelationship`・フレーム軸） | 差分中は血管が正の大きな値になるので、知らずに測ると**エッジの向きと `profileDomain` を取り違える** |
