...
>
> ✅ **A13-2（タスク・ランチャー）を実装し、実機検証 19/0（2026-08-16・§21.8）**。機能メニューに
...
| **A13** | **解析タスク UI**（ステップ・レール → ランチャー → 独立画面の 3 段） | — | 中 | 🟡 **A13-1 実装済・実機 30/30**（§21.7）／ **A13-2 実装済・実機 19/0**（§21.8）。A13-3 は未（急がない） |
...
| **A13-2** | 機能メニューに**タスク・ランチャー**を足す（カード一覧。未実装タスクは淡色＋「未対応」） — ✅ **実装済・実機 19/0（§21.8）** | `MainScreen` にダイアログを 1 つ追加するだけ…ではなかった（§21.8） |
...
### 21.8 A13-2（タスク・ランチャー）の受け入れ条件 — ✅ **実機検証 19/0 合格（2026-08-16）**
