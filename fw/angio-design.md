...
> 起票: 2026-08-14 ／ ステータス: **A1・A2・A3・A4・A9 実装済み（2026-08-14, ブランチ `feat/angio-a1`）／
> A5〜A8・A10〜A12 は未着手。実機検証はすべて未実施**
>
> **残件（次のセッションはここから）**
> 1. **実機検証が丸ごと未実施**。手順と合否条件は §5.8（A1）／§18。まず Rubo のサンプル
>    （96/137/70 フレーム）を取り込んで A1 の 9 項目を通すこと
> 2. **次の実装は A10（GSPS / QCA SR / エクスポート）を勧める**。A2・A4 の結果が
>    まだ**どこにも保存できない**（画面を閉じたら消える）ため、機能としての完成度がここで頭打ちになる。
>    併せて **A4 の中心線・エッジの手修正**（§17.1）を入れないと臨床 QCA としては使えない
