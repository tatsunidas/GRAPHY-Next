...
>
> **実機検証の到達点（合計 314 項目）**: A1 シネ 31 ／ A2 DSA 18 ／ A3 校正 11 ／
> A4+A10 QCA 20 ／ A4 手修正 34 ／ A9 RDSR 18 ／ **A4b ファントム 67（退行 0・設計目標に未達 17）**
> ／ **A13-1 ステップ・レール 30** ／ **A5b QLV 44** ／ **A6a 3D QCA 41（未達 2）**。
...
>
> ✅ **A6a（3D QCA 単一血管）を実装し、実機検証 41/0（目標未達 2）**: GNBP-XA-3 を生成（4 視点
...
>
> **残件（次のセッションはここから）**
> 1. **A6a の仕上げ** … ① 3D 中心線を既存 3D ビューア（`viewer3d/`）へ渡す（別ウィンドウなので
>    幾何の受け渡し経路が要る。今はダイアログ内の簡易プレビュー）。② 結果の保存（3D QCA SR）。
>    ③ 3D の %DS（参照径モデルの決定が要る。分岐部は A6b）。
>    **短縮の指標・ワーキングアングルの提案・断面の合成は実装＋実機検証済み（§10.2.5/§10.2.6）**
...
`automator/src/spike/xa3dQcaCheck.ts`（**合格 41 / 失敗 0 / 目標未達 2**）。
...
| **A6a** | 2D→3D 再構成・**3D QCA 単一血管**（射影幾何・バンドル調整・3D 表示） | A3, A4 | **大** | ✅ **実装済・実機 41/0（目標未達 2）**（§10.3.1）。`xaGeometry.ts` vitest 27 ＋ `xaRecon3d.ts` vitest 36 ＋ `Xa3dQcaDialog.tsx`。残: **既存 3D ビューアへの受け渡し**・保存（SR）・3D の %DS |
