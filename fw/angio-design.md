...
> 起票: 2026-08-14 ／ ステータス: **設計のみ・実装は未着手**
>
> **次に着手するもの: A1（XA シネ再生）**。理由 = 以下すべてが「XA のマルチフレームを
> 正しくフレーム列として扱えること」の上に乗る。現状は **XA を開いても先頭フレームしか出ない**
> （§2）ため、A1 が入るまで他フェーズは実機検証すらできない。
>
...
| フェーズ | 内容 | 依存 | 規模感 | 状態 |
| :- | :- | :- | :- | :- |
| **A1** | XA マルチフレーム展開・シネ再生・fps・メモリガード連携・**次元軸の提示（§5.7: `axes`/`stackAxis`）** | — | 中 | 🔴 未着手 |
| **A2** | DSA（`graphy-dsa:` ローダ・マスク選択・ピクセルシフト） | A1 | 中 | 🔴 |
| **A3** | キャリブレーション（C1/C2/C3・出自表示・永続化） | A1 | 小 | 🔴 |
| **A4** | QCA（中心線・エッジ・MLD/RVD/%DS・グラフ・手修正） | A2, A3 | **大** | 🔴 |
| **A4b** | GNBP-XA ファントムと精度検証ハーネス | A4 と並行 | 中 | 🔴 |
| **A5** | QVA / QLV | A4 | 中 | 🔴 |
| **A6** | 2D→3D 再構成（射影幾何・バンドル調整・3D 表示） | A3, A4 | **大** | 🔴 |
| **A7** | FFR インターフェース（host API H11/H12・色マップ） | A6 | 小 | 🔴 |
| **A8** | IVUS / OCT 同期 | A1（＋A4 の中心線） | 中 | 🔴 |
| **A9** | RDSR 読み取り・線量サマリ | — （独立） | 中 | 🔴 |
| **A10** | GSPS / QCA SR / MP4・PNG エクスポート | A2, A4 | 中 | 🔴 |
...
3. **QCA のエッジ検出法をどれで確定するか**（1 次微分極大 / 2 次微分ゼロ交差 / 両方選択可）。
   GNBP-XA-1 で両方測ってから決める。
