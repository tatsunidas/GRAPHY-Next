...
>
> **実機検証の到達点（合計 510 項目）**: A1 シネ 31 ／ A2 DSA 18 ／ A3 校正 11 ／
> A4+A10 QCA 20 ／ A4 手修正 34 ／ A9 RDSR 18 ／ **A4b ファントム 67（退行 0・設計目標に未達 17）**
> ／ **A13-1 ステップ・レール 30** ／ **A5b QLV 44** ／ **A6a 3D QCA 103（未達 2）** ／
> **A14 レポート統合 15** ／ **A13-2 タスク・ランチャー 20** ／ **A5a QVA 53（未達 1）**
> ／ **A6b 分岐部 46（未達 5）**。スパイクは `automator/src/spike/xa*Check.ts`。
>
...
> 真値の 2D 点列を入れれば ±1.7° で合う＝**実装の不備ではなく 2D 追跡の限界**。
> 代わりに **`daughterWiderThanMother` 警告**を足した（娘枝が母血管より太いのは物理的に無い）。
>
> **残件（次のセッションはここから）**
> 1. **QCA のエッジ検出の作り直し**（§16.4）。円柱モデルの当てはめにすれば径の絶対値が
>    直るが、実血管は円柱ではないのでモデル化誤差と交換になる。**設計判断が要る**。
>    ⚠️ 分岐部の未達 5 件（§21.4.3）も**追跡・エッジの問題**なので、ここと同じ根に繋がる
...
| 3 | ★**係数が径に依存する**ことを数値で残す | 3.6mm で 0.881 < 6mm で 0.908 |
