...
> 起票: 2026-08-14 ／ ステータス: **A1・A2・A3・A4（手修正含む）・A9・A10・A13-1 実装済み・
> 実機検証済み（ブランチ `feat/angio-a1`）／ A5〜A8・A11・A12・A14 は未着手**
>
> **実機検証の到達点（合計 229 項目）**: A1 シネ 31 ／ A2 DSA 18 ／ A3 校正 11 ／
> A4+A10 QCA 20 ／ A4 手修正 34 ／ A9 RDSR 18 ／ **A4b ファントム 67（退行 0・設計目標に未達 17）**
> ／ **A13-1 ステップ・レール 30**。スパイクは `automator/src/spike/xa*Check.ts`。
>
...
>
> ✅ **A13-1（ステップ・レール）は実装済み**（2026-08-15・実機 30/30・§21.7）。
>
> **残件（次のセッションはここから）**
> 1. **A5b（LV モノプレーン）** … 新しい順序の先頭。実データ `0009.DCM` あり。
>    **EF はスケール不変なので未校正でも検証できる**（§9.2.1）。ただし Kennedy 補正を入れると崩れる
> 2. **QCA のエッジ検出の作り直し**（§16.4）。円柱モデルの当てはめにすれば径の絶対値が
>    直るが、実血管は円柱ではないのでモデル化誤差と交換になる。**設計判断が要る**
> 3. **A14（レポート統合）** … QCA が実装済みなのに結果がレポートへ流れない（§21.5）
...
| 3D 中心線 | RMS < 0.5〜1.0 mm | ⚪ A6 未実装のため GNBP-XA-3 は生成していない |
| 3D 区間長（foreshortening 補正の実利） | 誤差 **< 3%**、かつ **2D 投影長より真値に近いこと** | ⚪ 同上。**「3D のほうが良い」を数値で示すのがこの行の役目** |
...
| **A5b** | **QLV モノプレーン**（RAO 30°・Area-Length・EF・壁運動） | A4 | 中 | 🔴 **実データあり**（§21.3） |
| **A5c** | QLV バイプレーン（Simpson 法） | A5b | 小 | 🔴 検証データ無し（§20-9） |
| **A6a** | 2D→3D 再構成・**3D QCA 単一血管**（射影幾何・バンドル調整・3D 表示） | A3, A4 | **大** | 🔴 **検証はファントム待ち**（§21.3） |
...
| **A14** | **レポート統合**（QCA/QVA/QLV の結果を既存レポートへ差し込む） | A4 | 小 | 🔴 **新規**（§21.5） |
...
| 層 | 手段 | 対象 |
| :- | :- | :- |
| 純ロジック | **vitest** | `xaCalibration.ts` / `qca.ts` / `dsa.ts` / `xaTasks.ts`（段の繋がり）/ `xaGeometry.ts` / `ivusSync.ts` / `recon3d.ts`。**mm に関わる計算は全部ここでカバーする**（UI から切り離して書く理由）。＋ `seriesLayout.ts` の **`axes`/`stackAxis` の既定値が現行動作と一致する回帰テスト**（§5.7 は CT/MR の表示を変えてはいけない） |
| backend | **JUnit** | `XaFrameExpander`（レイアウト）、`RdsrParseService`（SR トラバース） |
| 精度 | **`bench/` GNBP-XA** | §16.3 の合否基準。`measure_*.mjs` と同じ形の計測ハーネスを足す |
| 実機（standalone） | **automator spike** | `xaCineCheck.ts`（A1: フレーム数・fps・最終フレーム）／ `dsaCheck.ts`（A2: シフト量と残差）／ `qcaCheck.ts`（A4: ファントムでの %DS）／ `rdsrCheck.ts`（A9）／ `xaStepRailCheck.ts`（A13-1: **`data-state` で判定**。色や記号で判定しない） |
| ゲート | `/verify` | 変更のたび |
