...
- `frontend/src/viewer/ivusSync.ts`（未着手）: `d(f) = (f − startFrame)/frameRate × pullbackRate`、
  ランドマーク追加で区分線形。距離 → 経路上の位置は
  `qcaStraighten.ts` の `cumulativeLength()` / `sampleAlong()` の薄いラッパで足りる
- `sliceSync.ts` に **3 つ目のモード `"pullback"`** を足す（既存の双方向バス・ループ防止を再利用）
- 並置は `Viewer2DScreen.tsx` の `TileGrid`（§12 が言う「既存の複数ビューポート機構」の実体）
