...
> A13-2・A14 実装済み・実機検証済み（ブランチ `feat/angio-a1`）／
> **A5a も実装済み・実機検証済み**。A5c・A6b・A7・A8・A11・A12・A13-3 は未着手**
>
> **実機検証の到達点（合計 464 項目）**: A1 シネ 31 ／ A2 DSA 18 ／ A3 校正 11 ／
...
#### 21.4.2 実機で分かったこと（スパイクが見つけた 3 件）— 🟡 **実機検証は未完**

`automator/src/spike/xaBifurcationCheck.ts` を回して、**この機能を実データで通すには足りない**
ものが 3 つ見つかった。1 は直した。2・3 は残件。
...
| 2 🔴 | **遠位の区間を引いたつもりが、近位の計測が伸びた**（実測 89.5px を引いたのに 207px の計測が選ばれる） | 遠位の始点は近位の終点（＝カリーナ）と**同じ点**。Cornerstone の Length ツールは**既存注釈のハンドルの上でドラッグするとそれを掴んで動かす**。分岐部は「3 本が 1 点で出会う」形なので**必ず踏む**。UI 側の手当て（区間ごとに新規注釈を強制する／カリーナ側の端点を少しずらして引かせる）が要る |
| 3 🔴 | 先に解析したビューアを**閉じると登録が消える**（4/6 本しか残らない） | 登録簿は **BroadcastChannel で生きているウィンドウ間だけ**同期する（設計どおり・永続化しないのは automator の実行を跨がせないため）。単一血管（2 方向）では踏まなかったが、分岐部は**方向を跨いで 6 本**貯める必要があるので、**両方のビューアを開いたまま**にするか、登録簿の持ち主を変える必要がある。**タイルを 2 枚並べて 1 ウィンドウで済ませる**のが素直（次にこれを試す） |

> 🔑 **スパイクの作り方の教訓**: 最初は「最後に登録が 6 本あるか」だけを見ていたので、
> **どの区間で落ちたのか分からなかった**。区間ごとに「引いた区間を選べているか」
> 「2D QCA が結果を出したか」を検査に足した瞬間に 2・3 が切り分けられた。
> **合計だけを見る検査は、失敗したときに何も教えてくれない。**
