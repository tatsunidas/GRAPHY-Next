...
- **`ivusSyncStore.ts`**: 現在のプルバック距離・経路・幾何・ランドマークを持つ小さなストア
- **並置**: 左にアンギオ（シネ）、右に断層。`Viewer2DScreen.tsx` の `TileGrid` を使うか、
  既存の解析ダイアログ（QLV が画像を内側に描く形）に倣うかは未決
- 🔴 **未決: どのアンギオとどの断層を組にするか。** IVUS とアンギオは**別シリーズ**なので、
  シリーズを選ぶ導線が要る（既存の解析はすべて単一シリーズで完結しており、前例が無い）
- **経路の入力**: アンギオ上でプルバックの開始点と終了点を置く（§12.2）。
  中心線の追跡は `qca.ts` の `traceCenterline` を再利用できる
- 🔴 **§12.3 の限界を画面に出す**（心拍による縦方向運動を無視するので **±1〜2mm の近似**。
  **ステント端の位置決めに単独で使わない**）
