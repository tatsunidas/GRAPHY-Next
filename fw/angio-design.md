...
>
> **実機検証の到達点（合計 318 項目）**: A1 シネ 31 ／ A2 DSA 18 ／ A3 校正 11 ／
> A4+A10 QCA 20 ／ A4 手修正 34 ／ A9 RDSR 18 ／ **A4b ファントム 67（退行 0・設計目標に未達 17）**
> ／ **A13-1 ステップ・レール 30** ／ **A5b QLV 44** ／ **A6a 3D QCA 45（未達 2）**。
...
>
> ✅ **A6a（3D QCA 単一血管）を実装し、実機検証 45/0（目標未達 2）**: GNBP-XA-3 を生成（4 視点
...
#### 10.3.1 実機（画像から抽出した中心線）での結果 — 45/0/2

`automator/src/spike/xa3dQcaCheck.ts`（**合格 45 / 失敗 0 / 目標未達 2**）。
...
| **A6a** | 2D→3D 再構成・**3D QCA 単一血管**（射影幾何・バンドル調整・3D 表示） | A3, A4 | **大** | ✅ **実装済・実機 41/0（目標未達 2）**（§10.3.1）。`xaGeometry.ts` vitest 27 ＋ `xaRecon3d.ts` vitest 36 ＋ `Xa3dQcaDialog.tsx` ＋ `Qca3dSrWriter`（JUnit 9）。残: **既存 3D ビューアへの受け渡し**・3D の %DS |
