...
>
> **残件（次のセッションはここから）**
> 0. 🔴 **系統誤差の注記が A4c 前の文言のまま残っている**（コード側・§21.5 の囲み）。
>    i18n `report.analysis.caveat.diameterBias` / `xa3d.section.bias` / `qva.projectionCaveat`
>    （**ja・en 両方**）と backend の `Qca3dSrWriter` / `QvaSrWriter`。
>    ユーザに「径は 13% 過小」と読ませているが、**報告する径はもう密度計測**（§16.5.1）。
>    差し替える内容は「輪郭線は半値法・報告値は密度計測」「健常部は円と仮定」の 2 点。
...
> 🔴 **この文言は A4c（密度計測・§16.5.1・2026-08-18）以降は誤り。**（報告する径に 13% の
> 係数は乗らない。）**コード側が未更新**: i18n `report.analysis.caveat.diameterBias` /
> `xa3d.section.bias` / `qva.projectionCaveat`（ja・en 両方）と backend の
> `Qca3dSrWriter` / `QvaSrWriter`（＋ `Qca3dSrRequest` / `QvaSrRequest` の javadoc）。
> 差し替える文言は「輪郭線は半値法・**報告する径は密度計測**」「**健常部は円と仮定**している」
> の 2 点（§16.5.2）にする。**注記を付ける／付けないの線引き（下記）はそのまま有効**。
