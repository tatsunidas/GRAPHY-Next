...
| **A6b** | **3D QCA 分岐部**（polygon of confluence・分岐別参照径・分岐角） | A6a | 中 | ✅ **実装済・実機 46/0（未達 5）**（§21.4）。`xaBifurcation.ts` vitest 11 ＋ `Xa3dBifurcationDialog.tsx`。未達 5 件は**すべて 2D 追跡の原理的限界**が原因（§21.4.3） |
...
### 21.4 3D QCA 分岐部（A6b）— 単一血管の延長ではない ／ ✅ **実装済み・実機検証 46/0（未達 5）（2026-08-16）**
...
#### 21.4.3 🔴 残った誤差はすべて 1 つの原因 ——「投影で重なった血管を追跡してしまう」

未達 5 件（分岐角 遠位↔側枝 +9.8°／近位↔遠位 −9.9°／Finet −19.6pt／Murray −20.5pt／
側枝の 2D 参照径）は、**すべて側枝のカリーナ付近の 2D 追跡が母血管に乗っていること**から出る。
