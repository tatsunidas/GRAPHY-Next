...
>
> **実機検証の到達点（合計 199 項目）**: A1 シネ 31 ／ A2 DSA 18 ／ A3 校正 11 ／
...
> 2. **A13-1（ステップ・レール）** … 新しい順序の先頭。既存の QCA ダイアログの中だけで完結する
> 3. **A5b（LV モノプレーン）** … 実データ `0009.DCM` あり。**EF はスケール不変なので未校正でも
>    検証できる**（§9.2.1）。ただし Kennedy 補正を入れると崩れる
...
| **A13** | **解析タスク UI**（ステップ・レール → ランチャー → 独立画面の 3 段） | — | 中 | 🔴 **新規**（§21.2） |
...
| 段 | やること | 既存への影響 |
| :- | :- | :- |
| **A13-1** | ステップ・レールのコンポーネントとデータモデル（§21.6）を作り、**まず既存の QCA ダイアログに載せる** | ダイアログの中身だけ。導線は変えない |
...
export interface TaskStep {
  ...
  label: string;                 // i18n キー（ja/en 両方）
  state: TaskStepState;
  /** この段が無効化された理由（"invalid" のときだけ）。UI にそのまま出す。 */
  reason?: string;
  /** この段をやり直すと後続のどの段が無効になるか。再計算範囲の決定に使う。 */
  invalidates?: string[];
}
```
...
- 段の定義は**タスクごとの純データ**（`xaTasks.ts`）に置き、vitest で「`invalidates` が
  巡回していないこと」「全段が到達可能なこと」を検査する。UI を起動せずに守れる。
