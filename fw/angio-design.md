...
> 起票: 2026-08-14 ／ ステータス: **A1・A2・A3・A4（手修正含む）・A5a・A5b・A6a・A6b・A9・
> A10・A13-1・A13-2・A14 実装済み・実機検証済み（ブランチ `feat/angio-a1` →
> `feat/angio-a6b`）。A5c・A7・A8・A11・A12・A13-3 は未着手**
>
...
> ／ **A6b 分岐部 46（未達 3）**。**A4c（密度計測）で 2D ファントムは 67 → 93（未達 17 → 2）**。
...
>
> **残件（次のセッションはここから）**
> 1. **「健常部は円・病変部だけ非円形」のファントム**（§16.5.2）。形の独立性を
>    **実機で**示すのに要る。GNBP-XA-7 は一様断面なので使えない（vitest と
>    オフライン実験では担保済み）
> 2. **A13-3（独立画面）は急がない**。⚠️ タスクが 3 つ以上実装されてダイアログが実際に
>    破綻してからでよい（§21.2 の警告）。A13-1・A13-2 で段の可視化と入口はもう取れている
> 3. **GSPS の読み込み**（他社が書いた表示状態の適用）。書き出しは A10 で実装済み
> 4. **校正値の永続化**（現状メモリ上の Map。シリーズを閉じると消える）と MP4 書き出し
> 5. **GNBP-XA-2（DSA）の計測ハーネスが未着手**。データは生成済み（既知シフトを注入済み）
> 6. **web(BFF) モード（A12）は未確認**。backend の展開ロジックは共有済みだが動かしていない。
...
### 9.1 QVA（末梢・脳血管）— ✅ **実装済み・実機検証 53/0（未達 1）（2026-08-16）**
...
- **拡張（瘤）**: 最大径・拡張比・拡張率・瘤長・ネック径（近位/遠位）・偏心度・1.5 倍判定。
...
#### 10.3.1 実機（画像から抽出した中心線）での結果 — 103/0/2

`automator/src/spike/xa3dQcaCheck.ts`（**合格 103 / 失敗 0 / 目標未達 2**。2026-08-16 に
病変長の当てはめを直して 102/0/3 から更新）。
...
形の独立性を実機で示すには「**健常部は円・病変部だけ非円形**」のファントムが要る（未作成）。
現状は vitest（ビール則で作った合成断面）と bench のオフライン実験で担保している。
...
| **A4b** | GNBP-XA ファントムと精度検証ハーネス | A4 と並行 | 中 | ✅ XA-1/2/4/6/**7** 生成済・**実機 67 項目**（§16.4）。**非円形断面 XA-7 を追加（2026-08-17・§16.5）**。DSA ハーネスは未 |
| **A4c** | **エッジ検出の作り直し**（報告する径を密度計測へ。輪郭は半値法のまま） | A4b（XA-7 ✅） | 中 | 🟡 **方式決定済み・着手条件クリア**（§16.5）。実装は未（`qca.ts` に密度計測を足し、出自と σ̂ による退避を付ける） |
...
| 🔑 **差の % は径の系統誤差に依らない** | 3 本とも同じ係数（13% 過小）で縮むので比は不変。**%DS と同じ性質**であり、GNBP-XA-3 が式を満たさないこと（Finet −15.4% / Murray −10.8%）を実測で確認できる |
| **Medina 分類は出さない**（画面にもそう書く） | %DS 50% の閾値で機械的に決まるが**境界で跳ぶ** |
