...
> 起票: 2026-08-14 ／ ステータス: **A1・A2・A3・A4 実装済み（2026-08-14, ブランチ `feat/angio-a1`）／
> A5 以降は未着手。実機検証はすべて未実施**
>
> **残件（次のセッションはここから）**
> 1. **実機検証が丸ごと未実施**。手順と合否条件は §5.8（A1）／§18。まず Rubo のサンプル
>    （96/137/70 フレーム）を取り込んで A1 の 9 項目を通すこと
> 2. **次の実装は A9（RDSR）を勧める**。他フェーズと独立していて、A5〜A8 のような
>    大きな前提（3D 再構成・ゲーティング）を持たない
...
| **A9** | RDSR 読み取り・線量サマリ | — （独立） | 中 | 🔴 **次はこれを勧める** |
