...
#### 14.1.1 読み込み（2026-08-23 実装）— ✅ **JUnit 9 ／ vitest 10**
...
| **A3** | キャリブレーション（C1/C2/C3・出自表示・永続化） | A1 | 小 | ✅ 実装済・**実機 11/11**（§7.6）。**永続化も実装（2026-08-23・§7.4。実機は未）** |
...
| **A10** | GSPS / QCA SR / MP4・PNG エクスポート | A2, A4 | 中 | ✅ 実装済（§8.5 に含む）。**GSPS 読み込みも実装（2026-08-23・§14.1.1・JUnit 9 ／ vitest 10。実機は未）**。MP4 は未 |
