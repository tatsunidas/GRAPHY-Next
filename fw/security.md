...
> 📄 **設計は `fw/ai-routing-design.md` が正本**（2026-09-26 に**段 1〜4 完了**＝実装済み）。
> 以下は記録当時（2026-09-24）のメモ。**論点の答えは設計書側にある**ので、あちらを読むこと。
>
> 🔴 **安全の観点で変わった点 2 つ**:
> ① **同意の記憶鍵に宛先が入った**（`pluginId::providerId::scopeKey`）。それまでは宛先が
> 入っておらず、提供元が増えると**ある社への同意が別の社への送信を黙って許す**形だった。
> 同意ダイアログの送信先は**解決後のエンドポイント**を出す（定数ではない）。
> ② **提供元の設定は Electron main の `ai-providers.json` に置いた。backend の Settings に置かない**
> ——`GET /api/settings` が平文で全件返すため（この節の既存判断と同じ理由）。鍵は `secretStore` のまま、
> allowlist は `ai.provider.<id>.apiKey`（`<id>` は `^[a-z0-9-]{1,32}$` で検査）。
