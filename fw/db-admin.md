...
> 作成日: 2026-06-30
> ステータス: **Phase 1 実装済**（シリーズ削除・スタディ単位の患者編集・ドリルダウン木 UI・横断通知）。
> Phase 2/3（シリーズ統合・分割）は未着手。既存の患者編集・削除は実装済（`DbAdminService`）。
...
| **シリーズ削除** | 新規（Phase 1） |
| **シリーズ統合（N→1）** | 新規（Phase 2） |
| **シリーズ分割（1→N, 手動選択）** | 新規（Phase 3） |
| Patient→Study→Series ドリルダウン木 UI | 新規（Phase 1） |
