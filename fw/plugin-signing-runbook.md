# プラグイン署名鍵の運用手順（minisign）

> 作成日: 2026-07-28
> 対象: GRAPHY-Next の**公式プラグイン配布鍵**、および第三者プラグイン作者への案内。
> 設計: [`plugin-manager-design.md`](plugin-manager-design.md) §5.2（検証ロジック本体）

## いまどこまで終わっているか（2026-07-28 時点）

| # | 作業 | 状態 |
|---|---|---|
| 1 | アプリ側の署名検証の実装 | ✅ 完了（PR #71 / #73 / #74） |
| 2 | 公式配布鍵の生成（`minisign -G`） | ✅ 完了。鍵 ID **`98EA7C6BA2D50118`** |
| 3 | 公開鍵を `application.yml` の `trusted-keys` に登録 | ✅ 完了（PR #75）。`OfficialTrustedKeyTest` が退行を検知する |
| 4 | 自社の公式プラグイン用リポジトリへ secrets を登録 | ⏸ **配布を始めるときに実施 → §6** |
| 5 | 実際に署名付きリリースを出して動作確認 | ⏸ 同上 → §7 |

**秘密鍵とパスフレーズは生成した本人の手元にのみある**（リポジトリにも CI にもエージェントにも渡していない）。

### 誰の鍵の話をしているのか（混同しやすい）

この鍵は **当社が自分で配布する公式プラグイン専用**であって、プラグイン作者全体のためのものではない。

| 配布者 | 使う鍵 | secrets を登録するのは | 利用者から見た挙動 |
|---|---|---|---|
| **当社（公式）** | 公式鍵 `98EA7C6BA2D50118` | 当社（自社リポジトリのみ） | `verified`＝**確認画面なし** |
| **第三者の作者** | **作者自身の鍵**（当社は渡さない・預からない） | 作者が自分のリポジトリに | 初回は確認画面 → 以後 TOFU で自動検証 |
| 署名しない作者 | — | — | 毎回確認画面（導入自体は可能） |

第三者は当社の鍵を一切必要としない。むしろ**渡してはならない**（渡した相手は何でも公式扱いで
配れてしまう）。したがって ④⑤ は「当社が公式プラグインを配り始めるとき」に初めて発生する作業で、
それまでは何もしなくてよい。

### 覚えておくべき事実（実機で確認済み）

- **minisign 0.12 は `-H` 無しでも prehashed（algo `ED`）で署名する。** だからアプリ側は
  BLAKE2b-512 を自前で持っている（無いと実運用の署名を全部弾く）。
- **鍵 ID はバイト逆順・大文字で表示される。** アプリの同意画面も minisign CLI と同じ表記に
  揃えてあるので、そのまま見比べてよい。
- `minisign` は Ubuntu 22.04 / Pop!_OS のリポジトリには**無い**。公式のスタティックバイナリを
  `/usr/local/bin` 等に置く（この開発機は導入済み・`minisign 0.12`）。

## 0. 鍵の寿命 — 一度作れば使い続けられるか

**技術的には無期限に使える。** minisign（Ed25519）の鍵に有効期限の概念は無く、失効リスト（CRL）も
OCSP も無い。X.509 証明書のような「期限切れで一斉に動かなくなる」事態は起きない。

**ただし運用上、鍵の寿命を決めるのは次の 3 つ。**

| 事象 | 起きること | 復旧 |
|---|---|---|
| 秘密鍵を**紛失** | 以後の更新に署名できない。既存利用者は「前回は署名付き＝今回未署名」で**更新を拒否**される | 利用者側でアンインストール→再導入が必要（実質、全利用者に影響） |
| 秘密鍵が**漏洩** | 攻撃者が正規の署名を作れる。TOFU も信頼鍵も突破される | 鍵ローテーション（下記 §4）＋アプリ更新の告知 |
| 鍵を**変更**（意図的） | 旧アプリは新鍵を知らないため更新を拒否する | 移行期間中は §4 の手順で両鍵を許可する |

つまり **「作れば永続的に使えるが、失うと利用者全員に影響する」**。バックアップが本体。

---

## 1. 鍵を作る（1 回だけ）

```bash
minisign -G -p minisign.pub -s minisign.key
```

- パスフレーズを必ず設定する（空にしない）。
- `minisign.pub` は**公開してよい**（リポジトリにコミットする）。
- `minisign.key` は**秘密**。パスフレーズと合わせて漏らさない。

> ⚠ この鍵は Claude（エージェント）に生成させないこと。秘密鍵とパスフレーズは人が作り、人が保管する。

## 2. 保管する

- **オフラインのバックアップを 2 か所**（例: 暗号化 USB ＋ 金庫、あるいはパスワードマネージャの
  セキュアノート）。クラウドの平文ストレージに置かない。
- パスフレーズは鍵ファイルと**別の場所**に保管する。
- リリース CI に載せる場合は GitHub の repository secrets へ:
  - `MINISIGN_SECRET_KEY` … `minisign.key` の中身そのまま
  - `MINISIGN_PASSWORD` … パスフレーズ

## 3. アプリ側に信頼鍵として登録する

> ✅ **公式配布鍵は登録済み（2026-07-28）**: 鍵 ID `98EA7C6BA2D50118`。
> `backend/src/main/resources/application.yml` の `graphy.plugins.trusted-keys` を参照。
> 秘密鍵とパスフレーズは生成した本人が保管している（リポジトリにもエージェントにも渡していない）。

`backend/src/main/resources/application.yml`:

```yaml
graphy:
  plugins:
    trusted-keys:
      - "RWQ…（minisign.pub の base64 行）"
```

- ここに載った鍵で署名検証できたプラグインは `trust=verified` になり、
  **確認画面を出さずにそのまま導入される**（§5.2 の①）。
- 複数書ける。ローテーション時は新旧を並べる。
- 登録するまでは、自社配布のプラグインも `first-use` 扱い（確認画面が出る）。

## 4. ローテーション（漏洩時・定期更新時）

検証は「①信頼鍵 → ②台帳に固定した鍵（TOFU）」の順で走る（`PluginManagerService.evaluateSignature`）。
①が先に当たるので、**新鍵を `trusted-keys` に足したアプリでは、新鍵で署名した配布物がそのまま通る**
（固定鍵も新鍵に更新される）。

手順:

1. 新鍵を生成（§1）
2. `trusted-keys` に**新鍵を追加**（旧鍵は残す）→ アプリをリリース
3. 利用者にアプリ更新を促す
4. 十分に行き渡ったら、新鍵で署名したプラグインを配布し始める
5. 移行完了後、次のアプリリリースで旧鍵を削除

⚠ **順序を逆にしない。** 新鍵を知らない旧アプリは、新鍵署名の更新を②で「別の鍵」と判定して
**拒否**する。必ず「アプリに新鍵を配る」が先。

## 5. 第三者プラグイン作者への案内

**プラグインの配布者は基本的に第三者（不特定多数の開発者）**であり、各自が自分のリポジトリから
自分の鍵で配る。当社が関与するのは「アプリ側で検証する」ことだけ。
手順はテンプレート同梱の `examples/plugin-template/README.md` に記載済み。要点:

- 鍵は作者自身が作って持つ（当社は渡さない・預からない・失くしても復元できない）。
- 作者の secrets は作者のリポジトリに登録する。**当社の公式鍵は使わない**。
- 利用者側は**初回導入時にその鍵を記録**し、以後の更新で同じ鍵を要求する（TOFU）。
- したがって作者が鍵を失う／変えると、利用者は更新できなくなる。復旧はアンインストール→再導入。
- **署名を剥がした更新も拒否される**（乗っ取り側が `.minisig` を出さないだけで TOFU を回避できて
  しまうため。2026-07-28 に塞いだ）。

## 6. 公式鍵で署名する（＝**自社が配布するプラグイン**を出すときだけ）

> 🚫 **公式の秘密鍵を他人のリポジトリに置くことは絶対にない。** 置いた相手は何でも
> `verified`（確認画面なしで導入される）として配れてしまう。この節は
> **当社が所有するリポジトリから公式プラグインを配布する場合にのみ**適用する。
> 第三者作者は §5 のとおり**自分の鍵**を使う（当社の鍵は渡さない・預からない）。

自社の公式プラグインを配るリポジトリで、1 回だけ:

```bash
cd <当社が所有するプラグインのリポジトリ>
cp ~/graphy-signing/graphy-plugins.pub minisign.pub   # 公開鍵はコミットしてよい
git add minisign.pub && git commit -m "add signing public key" && git push

gh secret set MINISIGN_SECRET_KEY < ~/graphy-signing/graphy-plugins.key
gh secret set MINISIGN_PASSWORD            # プロンプトでパスフレーズを入力
```

自社で複数の公式プラグインを配るなら **organization secret** にまとめてもよい
（`gh secret set --org <当社の org> MINISIGN_SECRET_KEY < ...`）。組織外へは共有されない。

登録すると `release.yml` の署名ステップが有効になる（未登録なら自動でスキップされる仕様）。

**公式プラグインを 1 つも配らないうちは、この作業は発生しない。** その場合でも、
`trusted-keys` に公開鍵が入っていること自体は無駄にならない（将来配り始めた時点で即座に
`verified` になる）。

## 7. 動作確認（署名が効いているか）

1. プラグインのリポジトリでタグを push してリリースを作る
2. リリース資産に `<id>-<ver>.zip.minisig` と `minisign.pub` が付いていること
3. 手元で検証できること:
   ```bash
   minisign -V -p minisign.pub -m <id>-<ver>.zip -x <id>-<ver>.zip.minisig
   ```
4. GRAPHY-Next（standalone）で 環境設定＞プラグイン＞「プラグインの導入を許可する」を ON にし、
   `owner/repo` を入れて「GitHub から導入」→ **確認画面が出ずにそのまま導入されれば成功**
   （一覧の信頼欄が `verified` になる）

確認画面が出た場合は §8 を見る。

## 8. 症状別の対処

| 症状 | 原因 | 対処 |
|---|---|---|
| 確認画面が出る／信頼欄が `community` | 署名が無い、または鍵が `trusted-keys` に無い | リリースに `.minisig` が付いているか確認。付いているなら鍵 ID が `98EA7C6BA2D50118` か確認 |
| `signature check failed: ... does not match` | 配布物が署名後に差し替わった、または別の鍵で署名した | リリースを作り直す。心当たりが無ければ**乗っ取りを疑う** |
| `signature check failed: ... different key` | 前回と違う鍵で署名した（TOFU） | 意図的な鍵変更なら §4 のローテーション手順。利用者側の即時回避はアンインストール→再導入 |
| `... but this package has no signature` | 以前は署名付きだったのに今回未署名 | 署名を復活させる。秘密鍵を失っているなら §0 の「紛失」欄 |
| リリースに `.minisig` が付かない | secrets 未登録（ステップがスキップされた） | §6 |
| `minisign.pub がリポジトリにありません` でジョブが失敗 | 公開鍵をコミットしていない | §6 の 1 行目 |

## 9. やってはいけないこと

- 秘密鍵をリポジトリにコミットする（公開鍵だけコミットする）
- パスフレーズ無しの鍵を作る
- 同じ鍵を他用途（SSH・コード署名など）と兼用する
- 旧鍵を `trusted-keys` から消す前に、新鍵での配布を始める（§4 の順序）
