...
- 🚨 **パッケージ版で一度も動いたことが無かった（v0.2.2 まで）。** 実機確認をずっと
  `dev-desktop` でやっていたため、**dev にしか無い条件で通っていた**だけだった。原因は 2 つ
  重なっていた: ① 本番ビルドにしか焼かれない CSP の `script-src` が動的 `import()` を止める
  ② backend の CORS が **`Origin: file://` を 403** で弾く（`fetch` は Origin を送らないので
  アプリ本体の API は全部通り、**プラグインの `ui.js` だけ**が落ちる）。どちらも
  **dev では構造的に再現しない**。→ **プラグインの導線は配布物でしか検証にならない。**
  経緯と切り分けの型: `fw/security.md` §CORS。
  ✅ **v0.2.3（2026-08-24）で実機確認済み** — インストール版で `gemini-findings` が起動した。
  **プラグインの導線は配布物でしか検証にならない**、が今回の最大の教訓。
