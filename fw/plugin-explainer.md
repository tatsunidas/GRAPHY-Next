...
- **host API から結果を画面へ戻せない。** 2026-07-29〜30 に **H1（`getTargets()`＝表示中のスタディ/
  シリーズ/スライスの識別）・H2（`getViewState()`＝W/L・LUT・反転・affine）・H3（`getPixelData()`＝
  **校正済み画素**＝CT なら HU、表示 8bit ではない）を実装**した（0.1.9 以降）。
  画像処理プラグインは公式契約だけで書けるようになり、DOM 依存もキャンバス読み取りも不要。
  **残るのは H4＝処理結果の書き戻し**（オーバーレイ表示／派生シリーズとして保管庫へ）で、
  設計判断（派生 UID の生成・SEG との棲み分け・web での書き戻し先）が要るため未着手。
  → [`plugin-architecture.md` §7](plugin-architecture.md#7-host-api-の拡張優先度-高h1h3-実装済み)
  （H1 ✅ → H2 ✅ → H3 ✅ → **H4 書き戻し（未）**）
