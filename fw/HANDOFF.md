...
>
> ✅ **実機 3 日目（2026-08-22）で 3 面とも通った**。フュージョン・手による微調整とも OK。
> **画面まわりの残件は無い。**
>
> ✅ **S6b 完了（2026-08-22）。容積の k を実測で決めた**（正本 `fw/subtraction-design.md` §7.2.2）。
> 採点器は非公開側の `bench/score-volume-k.mjs`（GNBP-5N・測定 65 件・病変 22 個・腕 6 本）。
>
> 🔴 **既定の k を 40/50/60% → 25/35/50% に変えた。それまでの既定はどれも過小だった**
> （径 >= 20mm で 40% が 0.83・50% が 0.65・60% が 0.44）。
> 🔴 **単一の k で全径を正しくすることはできない**——径 >= 20mm は **k=35%**（真値比 0.95）、
> 12〜20mm は **k=50%**（0.94）、**12mm 未満はどの k でも過大に出るので数字を出さない**。
> 読み手は真ん中の列を読むので、**真ん中に 35% を置いた**。
> ⚠️ **S6a の「k=50%」は誤り**だった（n=3 の**平均**で、しかも径を混ぜていた。小さい病変の
> 過大評価が平均を押し上げていた）。**平均とサイズ混在が同時に効くと逆の結論が出る。**
> ⚠️ **母数の作り方を変えたのが効いた**——S6a は「増えた病変」しか見ていなかったが、
> `t00`（25 か所すべてが背景まで褪せた系列）を使えば「**消えた病変**」が 20〜25 個取れる。
>
> ✅ **§7.2 が心配していた「ピークの上振れ vs 平滑化による膨らみ」のトレードオフは空振り**
> だった（生／4mm／8mm のどれで測っても最適な k は 35%）。**計測は生の差分のままでよい。**
>
> **完了したフェーズ**: S0 / SP0 / S1 / SP1 / S6a / SP2 / H30〜H34 / SP3 / **S6b**。
> **残**: SP4（署名・配布）だけ。
>
> **中身は GNBP-5N で検証済み（S6a）** — 実測で設計を 3 つ変えた:
> ①**体部マスクは対数の軸で大津を採る**（線形だとマスクが視野の 0.2%＝熱い臓器だけになり、
> **マスクの失敗が「係数 2.95 倍」という正規化の失敗として現れる**）
> ②**既定の正規化を中央値比からロバスト回帰へ**（最悪値 10.5% → 3.4%。下駄は倍率だけでは
> 原理的に取れない。検出でもロバスト回帰だけがどの腕でも 11/16 を保った）
> ③**容積は k=50% が真値にいちばん近い**（0.96〜0.99・**n=3 の予備観察**。決定は S6b）。
> 採点器は非公開側の `bench/score-subtraction.mjs`（採点に使うのは `src/core/` の実物）。
>
> **見せ方**は host API を 5 つ足して本体から借りている（**どれもサブトラクションを知らない
> 汎用の能力**。本体に機能固有の語は 1 つも無い）:
> H30 専用ウィンドウ ／ H31 ビューポート ／ H32 3D-MIP ／ H33 メッシュ計測 ／
> H34 シリーズビューパネル（フュージョン込み）。
> 画面は **フュージョン / 差分 / 差分 3D-MIP の 3 面**（§11.2）。
>
> ⚠️ **フュージョン面は H34 をやめて H31 の 2 層に変えた**（実機の判断・2026-08-22）。
> 本体の `SeriesViewer` は**保管庫の実シリーズしか重ねられない**ので、
> **手で微調整したリサンプル結果**を映せない。いまは差分面と同じ経路で、
> 下地＝ターゲットの灰色／前景＝ソース（LUT ＋ 透過度スライダ）を host が 2 層で重ねる。
> **H34 は汎用の能力として残してある**（このプラグインは使っていない）。
> 微調整は設定パネルから **XYZ の平行移動 mm ＋ XYZ の回転角（度）**。フュージョンは
> 即座に追従し、差分と MIP は「更新」で作り直す。
>
> 🔴 **実機で 5 件直した（2026-08-21）。どれも「黙って壊れる」型だった**:
...
> **他のビューポートが全部消える**——プラグインの 2D 3 面も本体のタイルも。エラーは出ない。
> → `enableElement()` で足す形へ（§14-20）
> ② **本体の LUT 名は「登録済み」ではない**。`Hot_Iron` 等はユーザーが LUT ダイアログで 1 度
> 使うまで cornerstone に登録されず、指定しても**黙って灰色**。発散色は host が
> `"divergent"` として必ず登録する（§14-21）
> ③ ①を直すときに**ツールグループごと落として 3D が回せなくなった**。プラグイン専用の
> ツールグループを作って戻した（2D の面にも無かったので同時に付けた）
> ④ **「減ったところ」を MIP しても何も出ない**（最大値投影なので負のかたまりは必ず負ける）。
> 拾う符号で投影を選ぶ: 増えた→MIP / 減った→**MINIP** / 両方→**絶対値の MIP**。
> 併せて成分の一覧に**符号の列**を足した（§11.3.1）
>
> 🔴 **目的（奏効を見る／増悪を見る）でソースとターゲットを入れ替えない。**
> 時間順は「前→後」で固定し、**拾う符号だけ**を変える。入れ替えて全部を正にすると
> 符号の意味（正＝増えた）が失われ、保存した派生シリーズを後から読めなくなる（§11.3.1）。
>
...
>
> **実機 3 日目（2026-08-22）の結果**: **3 面とも通った。フュージョンも手による微調整も OK。**
実機で直した 3 件（前景の LUT が灰色のまま／透過度のつまみが掴めない／単位が折り返す）も解消。
そのうえで出た問いが「**容積の k は変えられますか**」で、**変えられるようにした**（§7.2.1）。

**実機 2 日目（2026-08-21 夜）の結果**: 3 面レイアウトは出た。**差分 3D-MIP は描けている**
> （閾値後・増えたところ）。**フュージョン面だけ空**で、原因は上記の H34 のプロバイダ問題。
> 数字は 倍率 0.9796 / 下駄 88.26 / 体部マスク 9.9%（位置合わせ 剛体のまま）。
> ⚠️ 「キャプチャのようになった」という報告のうち、**フュージョン面が空**だった件は
> これで説明が付く。他に崩れている点があれば次回確認する。
>
...
> | 体部マスク | **9.9%** |
> | かたまり | **32 個**、最大の mL **74.9 / 52.1 / 31.6**、重心 **(-103, 15, 73)** |
> | フュージョン面（H34・**未検証**） | ターゲットにソースが重なって出る。W/L バー・スライダが付く |
> | 差分面（H31） | 発散色（負=青 / 0=暗灰 / 正=赤） |
> | 3D-MIP（H32） | 塊が浮かぶ。左ドラッグで回る |
> | メッシュ列（H33） | mL と長径が入る。**この列は k と無関係**（検出の閾値で切ったラベルから作る） |
| 容積の k | 設定パネルの「容積」節。`40/50/60` を書き換えると**その場で**表の mL が変わる（再計算は要らない） |
| MIP の白黒反転 ✅ | 面の右上の「白黒反転」。**実機確認済み**（2026-08-22）。向きは保たれ、四隅の楔も点々も出ない。🔴 **MINIP とは別物**（投影は最大値のまま） |
>
> ✅ **H34 の「怪しい」は的中して、直した**（`cadeca2`）。独立した `createRoot()` は本体の
> プロバイダの外なので `useI18n must be used within I18nProvider` で即死していた。
> **本体のツリーの中から `createPortal` で差し込む**形に変更（ポータルは DOM の行き先だけを
> 変え、React の文脈はツリー上の位置から引く）。エラー境界も入れたので、次に React の描画で
> 落ちたら**面に理由が出る**（`await` の try/catch では React の描画中の例外は捕まらない）。
> **この版はまだ未検証。**
> 奏効側も見るなら ソース `t25` / ターゲット `t00` / 拾う符号**「減ったところ」**。
>
...
1. **GSPS の読み込み**（書き出しは A10 で実装済み）／**校正値の永続化**（現状メモリ上の Map で
   シリーズを閉じると消える）／**MP4 書き出し**。いずれも独立した小物。
2. **web(BFF) モード（A12）**。ランチャーは web では XA を理由付きで無効化している。
   最後の一手が `xaTaskCatalog.ts` の `standaloneOnly` を外すこと。
3. **A13-3（独立画面）は急がない**（§21.2 の警告）。A5c（LV バイプレーン）は**実データが無い**。
...
⚠️ **更新通知メールの実機作業は 2026-07-28 に完了済み**（下のログ参照）。
