...
| | |
| :- | :- |
| origin/main | **`d83a592`**（`docs(handoff): 中断時点の引き継ぎブロックを冒頭に置く`）|
| 未 push の作業 | **無い**。ローカルのブランチはすべて main にマージ済み。**open な PR も 0 件** |
| ローカルブランチ | `feat/angio-a6b`（＝main と同一。この作業ツリーが立っているだけ）／`release/0.1.16`（`gn-fixes` ワークツリーが使用中）|
| リモートブランチ | `origin/main` **のみ**（マージ済みブランチは削除済み）。⚠️ 古い clone には remote-tracking ref が大量に残っていることがあるので `git fetch --prune` で消す |
| 公開デモ | 夜間リセットで main の最新版になる。**アンギオ一式（QCA・QVA・3D QCA・分岐部・ランチャー）が載っている** |

⚠️ **以下 2 つは中断した PC 固有の事情**（`gn-h5` / `gn-fixes` ワークツリーがある機）。
**別の PC では当てはまらない**ので、まず `git worktree list` で自分の環境を確認すること。

- **`main` は `gn-h5` ワークツリーがチェックアウトしている**ので、その機では `GRAPHY-Next`
  直下で `main` に立てられない。main を進めるときは「作業ブランチでコミット →
  `gn-h5` で `git merge --ff-only` → push」の順で行う。
- **`gn-fixes` ワークツリーでは別セッションの Vite dev server が動いている**
...
5 件の障害を直して **46/0（未達 5）** で通した。詳細は
[`fw/angio-design.md`](angio-design.md) **§21.4.2 / §21.4.3**。

未達 5 件は**すべて 1 つの原因**——「投影で母血管と重なった側枝を追跡してしまう」。
再構成の数学は真値の 2D 点列を入れれば ±1.7° で合うので**実装の不備ではない**。
壊れているときに黙らないよう `daughterWiderThanMother` 警告を足してある。

退行が無いことは A6a（103/0/2）と 2D ファントム（67/0/17）を回して確認済み。
