...
> 更新日: 2026-08-18（**再開はすぐ下の「▶ ここから再開」から**）
> 主題: **アンギオ（XA）**。**A4c は完了**（密度計測・2D 93/0/2 ＋ 病変の形 33/0/0）。
> **GNBP-XA-2（DSA）の計測ハーネスも完了**（体動の自動推定 17/0/0・誤差 0.000px）。
> 次の一手は **GSPS 読み込み／校正値の永続化／MP4 書き出し**（独立した小物）。
...
### 3. このセッションで閉じたこと（2026-08-18 その 2）— **DSA の体動を初めて数値で測った**
