...
1. 🔴 **QCA のエッジ検出の作り直し**（`fw/angio-design.md` §16.4）。**設計判断が要るので
   ここから**。径の絶対値が一貫して 13% 過小に出るのは半値法を円柱投影に当てた帰結で、
   円柱モデルの当てはめにすれば直るが**実血管は円柱ではない**のでモデル化誤差と交換になる。
   ⚠️ A6b の未達 5 件（§21.4.3）も追跡・エッジの問題なので**同じ根**。ここを触るなら
   両方を一度に良くする設計にできないかを先に検討する価値がある。
   ⚠️ 着手前に §16.4 と §21.4.3 を両方読むこと。
