...
> 次の一手は **サブトラクション 機能**。**調査と設計は完了**（正本 `fw/subtraction-design.md`）。
> 🔴 **公開できないことになったため、非公開リポジトリの「プラグイン」として作る**（同 §15）。
> 動線は「プラグイン導入 → 2D ビューア**「解析」メニュー**に項目が増える」。
> **公開リポジトリ側の作業 2 つは完了**——✅ **S0**（GNBP-5N に強度の腕 9 系列。
> `d080` 投与量 ×0.8 ／ `s125` 格納スケール ×1.25 ／ `b010` 下駄 +10%。**基本 18 系列は md5 一致で無傷**）
> と ✅ **SP0**（surface `viewer2d.menu.analysis` ＋ 解析メニューでの出所の明示）。
> 計算系の host API は **H10/H21/H28 で既に足りている**（`15d287e`）。不足は「見せる」側だけ
> （H30〜H33）。
> ✅ **未確認だった 2 件は決着（2026-08-21）**: ①H30〜H33 は**公開 host API として出してよい**
> （箱は公開・中身は非公開）②目的は**「製品として今はまだ公開しない」で、秘匿ではない**。
> ②の帰結で構成は変えない ＝ 計算はフロント側のまま ＝ **web でも動く**。
> ✅ **S1 完了**: 非公開リポジトリ
> [`graphy-next-plugin-series-subtraction`](https://github.com/tatsunidas/graphy-next-plugin-series-subtraction)
> を作成し（private・id `series-subtraction`）、純ロジック 4 本（`stats` / `normalize` /
> `subtract` / `smooth` / `threshold`）＋ テスト 60 件を push 済み（`22e78fc`・依存パッケージ 0）。
> **測って設計へ返したことが 2 つ**——🔴 `robustLinear` は平滑化なしだと傾きが 2 割以上縮む
> （回帰の希釈。§3.4）／🔴 検出の画像で採ったピーク値を計測の画像の閾値に流用すると
> 等値面が体部全体へ漏れる（§6.3・`refineSeeds`）。
> ✅ **テンプレートの `.d.ts` を本体の契約に追いつかせた**（`b34a942`）: H10/H21 が丸ごと
> 抜けていた（＝作者のエディタからは存在しないように見えていた）。goTo / selectRoi /
> `viewer2d.menu.analysis` / `PluginModule` も追加。**ずれを検知するテストを足した**
> （`frontend/src/plugins/pluginTemplateTypes.test.ts`。host API を足すと落ちる）。
> ✅ **SP1 完了**（`74f7796`）: 解析メニュー → 側パネル → 対象選択 → 位置合わせ（H21）→
> 正規化 → 減算 → 平滑化 → z 閾値 → 連結成分 → 等値面容積 → `showOverlay`（H4a）で重畳。
> **host API は 1 つも足していない。**
> ✅ **S6a 完了**（`5e726a9`・テスト 77 件）: GNBP-5N で中身を採点し、**実測で設計を 3 つ変えた**。
> ① **体部マスクは対数の軸で大津を採る** — SPECT のカウントは桁で広がるので線形だと閾値 5025 で
> **マスクが視野の 0.2%＝いちばん熱い臓器だけ**になり、比の分母が病変そのものになって
> **係数が 2.95 倍**という値になる。**マスクの失敗が正規化の失敗として現れる**ので気付きにくい。
> ② **既定の正規化を中央値比からロバスト回帰へ** — 最悪値が 10.5% → 3.4%。どの場面でも最良では
> ない（倍率だけ・奏効では中央値比が僅かに良い）が、**下駄は倍率だけでは原理的に取れない**。
> 検出でもロバスト回帰だけがどの腕でも 11/16 を保った（中央値比 9/16・正規化なし 8/16）。
> ③ **容積は k=50% が真値にいちばん近い**（0.96〜0.99）。ただし **n=3 の予備観察**で決定は S6b。
> 採点器は非公開側の `bench/score-subtraction.mjs`（採点に使うのは `src/core/` の実物）。
> 併せて公開側の `bench/nmVolume.mjs` に読み取り器を切り出し、**JS ハーネスを Windows で
> 動くように直した**（`fileURLToPath` / `esbuild.cmd` / `pathToFileURL` の 3 か所）。
> ✅ **SP2 完了**（`62527fa`・テスト 82 件）: `saveDerivedSeries`（H4b）で差分を派生シリーズに
> 残せるようになった。**3D ビューアで開いて表示モードに MIP を選べば要件 3 に届く**（host API の
> 追加ゼロ）。既定は**閾値後**の保存——生の差分を MIP するとレイ上の最大値が拾われて
> 背景ノイズの最大値が全面に出る。
> 🔴 **SP2 で見つけた罠**: 重畳も保存も「タイルのスライス番号」で位置を決めるのに、派生シリーズの
> 幾何はタイルの `imageIds` から引かれる。プラグインのボリュームとタイルのスタックで並びが違うと
> **幾何は正しいのに中身が別スライス**という壊れ方をし、**見て気付けない**。NM 多フレームは本体が
> 法線に沿って積み直すので一致の保証は契約として無い。数枚を `getPixelData` で読み比べてから書く
> （`verifyGridAlignment`）。合わなければ保存も重畳もしない。
> ⚠️ **要件 5（メッシュ）は SP2 では届かない**——本体のメッシュ化の入口は Cornerstone の
> セグメンテーションで、H4b が書けるのは画像シリーズだから。**H33 が要る理由がこれ**。
> 容積の数値そのものはプラグインが出している。
> ✅ **H30〜H33 実装済み（公開側・テスト 942 件）**: プラグイン専用ウィンドウ（H30）／
> 値ボリュームの 2D ビューポート貸し出し（H31）／3D の MIP・MINIP・VR（H32）／
> マスクのメッシュ化と計測（H33）。**いずれもサブトラクションを知らない汎用の能力**で、
> 本体側に機能固有の語は 1 つも入っていない。
> 🔴 **H30 は「別ウィンドウ」にできなかった**——Electron の `openViewer()` は別の JS
> コンテキストで `container: HTMLElement` を渡せず、`window.open` でも RenderingEngine が
> 文書ごとに要って単一エンジン前提が崩れる。**同じ文書の中に浮かべる窓**にした（§15.4.1）。
> 🔴 **H31 の幾何委譲先は「タイルのスタック」に固定**した。重畳（H4a）と派生シリーズ保存（H4b）が
> 既に同じ並びでスライス番号を解釈しているので、3 者が同じ 1 本を見るようにした
> （`getStackImageIds()` を viewer command に追加）。
> **H33 は実測でテスト済み**（球の体積が理論値の 0.9〜1.1 倍・セグメント分離・患者座標）。
> **H30〜H32 は描画を伴うので実機未検証。**
> 🔴 **溜まっている目視**: SP1（解析メニューに出る／押して計算が通る）・SP2（保存した差分が
> 3D で MIP になる）・H30〜H32（窓とビューポートが出る）。GNBP-5N の `t20-id-tex` と
> `t25-id-tex`（`bench/phantom/` に生成済み）を取り込んで 2 タイルに開けば**まとめて 1 度で通せる**。
> プラグインは `desktop/plugins/` へ導入済み（反映には再起動が要る）。
> ✅ **SP3 完了**（非公開 `91e59bd`・テスト 86 件）: **要件どおりの姿**になった。
> 4 面（ソース／ターゲット／差分／差分 3D-MIP）＋ 設定パネルの専用ウィンドウで、
> 実行すると 閾値 → 連結成分 → メッシュ → 容積 まで自動で通る。**描画は 1 行も自前で書いていない。**
> ソース面は既定で「位置合わせ・正規化後」、**「元のまま表示」トグル**あり
> （位置合わせが効いているかを目で確かめられないと偽陽性を疑えないため）。
> 容積は 2 列出す（等値面 k=40/50/60% と メッシュ）——一致しないのが正常で、
> 片方だけ出すとそれが真値だと読まれる。
>
> 🔴 **次にやること: 実機での目視。計算は検証済みだが画面は 1 度も見ていない。**
> SP1（解析メニューに出る／押して計算が通る）・SP2（保存した差分が 3D で MIP になる）・
> H30〜H32（窓と 4 面が出る）・SP3（自動の容積）を、GNBP-5N の `t20-id-tex` と
> `t25-id-tex`（`bench/phantom/` に生成済み）を 2 タイルに開いて**まとめて 1 度で**通せる。
> その後は **S6b**（容積の k を確定）と **SP4**（配布）。
