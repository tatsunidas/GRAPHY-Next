...
>
> 🔴 **実機 1 日目（2026-08-21）で 4 件直した。どれも「黙って壊れる」型だった**:
...
>
> ## ▼ 明日の実機テスト（プラグインは `desktop/plugins/` へ導入済み・**要再起動**）
>
> 🔴 **前回のキャプチャの件が未解決のまま**: 「キャプチャのようになってしまいました」と
> 報告があったが**画像が添付されておらず、何が崩れたか不明**。3 面化で消えた可能性もあるが、
> **別の問題なら見落とす**。まず現象の有無を確認すること。
>
...
> | メッシュ列（H33） | mL と長径が入る |
>
> ⚠️ **H34 が最も怪しい**: 本体の React ツリーの**外**でルートを立てるので、`SeriesViewer` が
> 親の文脈（Provider 等）に依存していると動かない。**面に理由が赤字で出る**ようにしてある。
