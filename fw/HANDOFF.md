...
> 🔴 **SP1/SP2 の目視はまだ**——backend が `series-subtraction` を発見し `ui.js` を 200 で配信する
> ことは確認した（`desktop/plugins/` へ導入済み）が、**解析メニューに出る／押して計算が通る／
> 保存した差分が 3D で MIP になる**は未目視。GNBP-5N の `t20-id-tex` と `t25-id-tex`
> （`bench/phantom/` に生成済み）を取り込んで 2 タイルに開けば通せる。
> **次は H30〜H33（公開側）→ SP3。**
