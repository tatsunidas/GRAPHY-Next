...
> 更新日: 2026-08-17（**作業中断。再開はすぐ下の「▶ ここから再開」から**）
> 中断時点の主題: **アンギオ（XA）**。次の一手は **A4c＝`qca.ts` に密度計測を実装**（方式は決定済み）。

---

## ▶ ここから再開（2026-08-17 中断時点のまとめ）
...
1. 🟡 **QCA のエッジ検出の作り直し（A4c）→ 方式は決定済み（2026-08-17・`fw/angio-design.md` §16.5）**。
   対照実験（`bench/experiment_qca_edge.py`）で 4 方式を突き合わせ、**報告する径は密度計測**
   （−ln T の横積分＝μ·A。**形を仮定しない**）、**μ は健常部で 1 回だけ円柱を当てはめて得る**、
   **輪郭は半値法のまま**、と決めた。当初案の「円柱モデルの当てはめ」は**採らない**。
   ✅ **着手条件（非円形断面のファントム）は満たした**——**GNBP-XA-7** を追加し、
   **M3 は形に依らず 0.4% 以内**・**M1 の係数は 0.745〜0.918 まで動く**（＝定数ではない）ことを
   測定で示した。**残っているのは `qca.ts` への実装だけ**（密度計測の追加、出自への記録、
   σ̂ による半値法への退避）。
   ⚠️ **A6b の未達 3 件（§21.4.3）はこれでは直らない**——あれは追跡が重なった母血管に乗る
   問題で、密度計測はむしろ重なりに弱い。**同じ根ではないので一緒に直そうとしない。**

   <details><summary>A4c の実装手順（再開したらここから）</summary>

   1. `frontend/src/viewer/qca.ts` に密度計測を足す。`findEdgesInProfile` の**隣**に置き、
      **置き換えない**（輪郭は半値法のまま使う）。入力は同じ 1D プロファイル。
      `area = Σ max(0, −ln(T/T_bg)) · step / μ`、`equivalentDiameter = 2√(area/π)`。
      🚨 背景 `T_bg` はプロファイル両端の中央値で取る（`experiment_qca_edge.py` と同じ約束）。
   2. μ は**健常部で 1 回だけ**円柱を当てはめて得る。当てはめは
      `experiment_qca_edge.py` の `m4_cylinder_fit`（r と μ の 2 次元グリッド探索）を移植すれば足りる。
      🚨 **病変部で μ を取り直さない**（形の仮定が病変に及ぶ）。
   3. `QcaProvenance` に「径をどちらの方式で出したか」を足す。
      `profileNoiseScale()` の σ̂ が大きいときは半値法へ退避し、**退避したことを出自に残す**。
   4. vitest: XA-7 と同じ断面（円・楕円 2 種・三日月・D 型）の合成プロファイルで
      **密度計測が面積等価直径を返すこと**を固定する。⚠️ **箱型では差が出ない**ので使わない
      （§16.4 の失敗の再演になる）。
   5. 実機: `xaPhantomCheck`（67/0/17 が基準）＋ **XA-7 を回すスパイクを新設**。
      `experiment_qca_edge.py` の表がそのまま期待値になる。
