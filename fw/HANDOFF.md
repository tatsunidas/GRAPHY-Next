...
> **host API は 1 つも足していない。** テスト 73 件。
> 🔴 **実機検証は「読み込まれる」ところまで**——backend が `series-subtraction` を発見し
> `ui.js` を 200・59505 バイトで配信することは確認した（`desktop/plugins/` へ導入済み）。
> **解析メニューに出る／押して計算が通る**は未目視。
> **次は S6a**（GNBP-5N で定量検証）。その最初に上の目視を通す。
