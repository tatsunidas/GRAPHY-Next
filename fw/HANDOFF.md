...
> **画面まわりの残件は無い。**
>
> 🔴 **次はここから: S6b（容積の k を確定）**。実機から出た問いが
> 「**閾値 k は変えられますか**」で、**変えられるようにした**（§7.2.1・k の欄はプラグインの
> 「容積」節。10〜95%・最大 5 本・区切りは `/` `,` 空白のどれでもよい）。
> k を変えると**その場で測り直す**（位置合わせからやり直さない）ので、
> **GNBP-5N を開いて k を振るだけで S6b の材料が採れる**状態になっている。
>
> **完了したフェーズ**: S0 / SP0 / S1 / SP1 / S6a / SP2 / H30〜H34 / SP3。
> **残**: S6b（容積の k を確定）と SP4（配布）。
>
