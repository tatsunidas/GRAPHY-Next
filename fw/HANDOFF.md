...
> 主題: **アンギオ（XA）**。**A4c（密度計測）は実装済み・実機 93/0/2**。次の一手は
> 「健常部は円・病変部だけ非円形」のファントム（§16.5.2）。
...
### 2. このセッションで閉じたこと（2026-08-18）— **A4c を実装した**
...
- 🔴 **実装して初めて分かった限界（§16.5.2）: 「形を仮定しない」は病変部についてだけ。**
  ...
  健常部の当てはめが必ず外れる）。**「健常部は円・病変部だけ非円形」のファントムが要る**。
...
1. 🟡 **「健常部は円・病変部だけ非円形」のファントム**（§16.5.2）。
   **A4c 自体は 2026-08-18 に実装済み**（実機 93/0/2）。残っているのはこれだけで、
   **形の独立性を*実機で*示す**のに要る。GNBP-XA-7 は各フレームが一様な非円形断面なので
   使えない（健常部の当てはめが必ず外れる）。いまは vitest（ビール則で作った合成断面）と
   `bench/experiment_qca_edge.py` のオフライン実験で担保している。
   ⚠️ **A6b の未達 3 件（§21.4.3）はこれとは無関係**——あれは追跡が重なった母血管に乗る
   問題で、密度計測はむしろ重なりに弱い。**同じ根ではないので一緒に直そうとしない。**
2. **GNBP-XA-2（DSA）の計測ハーネス**。データは生成済み（既知シフト注入済み）で、
   **設計判断が要らない**ので手が空いたときに向く。
3. **GSPS の読み込み**（書き出しは A10 で実装済み）／**校正値の永続化**（現状メモリ上の Map で
