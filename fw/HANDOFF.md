...
> 🔴 **SP1 の目視はまだ**——backend が `series-subtraction` を発見し `ui.js` を 200 で配信する
> ことは確認した（`desktop/plugins/` へ導入済み）が、**解析メニューに出る／押して計算が通る**は
> 未目視。GNBP-5N の `t20-id-tex` と `t25-id-tex`（`bench/phantom/` に生成済み）を 2 タイルに
> 開いて通すこと。**次は SP2 か H30〜H33。**
