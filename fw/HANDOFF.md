...
> ✅ **アンギオの積み残しを閉じた（2026-08-23）**: `26dcff0`（注記を径の測り方で切り替え）を
> **実機で確認した**——**3D QCA 116/0/1 ／ QVA 67/0/0 ／ 2D 93/0/2**（退行 0）。
> 🚨 **確認のためにスパイクへ検査を足した**。それまで「画面の文言」と「SR の中身」を
> **誰も見ていなかった**ので、今回直した種類のバグ（数値は新方式・説明は旧方式）は
> **素通りしていた**。次は **GSPS 読み込み／校正値の永続化／MP4 書き出し**。
...
2. **GSPS の読み込み**（書き出しは A10 で実装済み）／**校正値の永続化**（現状メモリ上の Map で
   シリーズを閉じると消える）／**MP4 書き出し**。いずれも独立した小物。
