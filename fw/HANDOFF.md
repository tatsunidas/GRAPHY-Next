...
| | |
| :- | :- |
| origin/main | **`55161d5`**（`docs(subtraction): 面の見出しの書き方を §11.2.1 として決める`）|
| 非公開プラグイン | `graphy-next-plugin-series-subtraction` の `origin/main` が **`a2814c6`**。**作業ツリーは clean・未 push 無し**。ローカルは `~/graphy-workspace/graphy-next-plugin-series-subtraction` |
...
**検証**: 本体 typecheck / vitest 944 / build、プラグイン `node --test` 117、
