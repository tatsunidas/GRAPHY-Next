...
> 更新日: 2026-08-23（**再開はすぐ下の「▶ ここから再開」から**）
> 主題: **サブトラクション（シリーズ間の経時差分）を作った**。正本 `fw/subtraction-design.md`。
> 非公開リポジトリ
...
| | |
| :- | :- |
| origin/main | **`76aa98c`**（`docs(subtraction): 前段の失敗を後段の結論にすり替えない`）|
...
### 2. このセッションで閉じたこと（2026-08-22）— **サブトラクションを仕上げ、S6b を決着させた**
...
### 3. 前のセッションで閉じたこと（2026-08-21）— **Fusion のカーソル値を二段にした**
...
### 4. 前のセッションで閉じたこと（2026-08-19〜20）— **A4c の後始末（注記の食い違い）**
...
### 5. 前のセッションで閉じたこと（2026-08-18）— **A4c を完了した**
...
### 6. 前のセッションで閉じたこと（2026-08-18 その 2）— **DSA の体動を初めて数値で測った**
...
3. **web(BFF) モード（A12）**。ランチャーは web では XA を理由付きで無効化している。
   最後の一手が `xaTaskCatalog.ts` の `standaloneOnly` を外すこと。
4. **A13-3（独立画面）は急がない**（§21.2 の警告）。A5c（LV バイプレーン）は**実データが無い**。
