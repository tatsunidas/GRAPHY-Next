...
| | |
| :- | :- |
| origin/main | **`b58a394`**（`fix(angio): 分岐部 QCA の実機検証を閉じる（A6b・46/0・未達 5）`）|
| 未 push の作業 | **無い**。ローカルのブランチはすべて main にマージ済み |
...
⚠️ **`main` は `gn-h5` ワークツリーがチェックアウトしている**ので、この作業ツリー
（`GRAPHY-Next` 直下）では `main` に立てられない。main を進めるときは
「作業ブランチでコミット → `gn-h5` で `git merge --ff-only` → push」の順で行う。

⚠️ **`gn-fixes` ワークツリーでは別セッションの Vite dev server が動いている**
...
**アンギオ以外**では、`CLAUDE.md` の「現在地」にある🔴 **更新通知メールの実機作業**が
保留のまま（コードは 3 リポジトリとも完了・**公開デモ機での適用が未実施**）。
⚠️ **旧 DB に残る登録者の移行を夜間リセットより前に**行う必要がある。
