...
軽量な自前 i18n（依存追加なし）。`frontend/src/i18n/`:
- `i18n.tsx`: `I18nProvider` / `useI18n()` → `{ t, locale, setLocale }`。locale は localStorage
  (`graphy.locale`) に保存し即時切替（既定はブラウザ言語→日本語）。
- `ja.ts` / `en.ts`: フラットキーの辞書（キーは両者一致）。`t("key", { var })` で `{{var}}` 補間。
- 言語切替は環境設定「一般 > 言語」（i18n コンテキスト直結）。
- Electron スプラッシュも localStorage の locale で日英切替、起動進捗は `step` でローカライズ。

**規約（重要）**: 今後フロントに追加する**全ての UI 文字列・メッセージは `t()` 経由**にし、
日英の辞書キーを `ja.ts`/`en.ts` の両方へ追加する。ハードコード文字列を残さない。
設定項目（registry）の label/help/option も i18n キーで定義する。
