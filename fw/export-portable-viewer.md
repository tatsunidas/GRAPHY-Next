...
- ランタイム本体（方式 A・vanilla TS・React 本体とは別バンドル）:
  ...
  - `frontend/portable/src/viewer.ts` … **マネージャ**（共有 RenderingEngine＋共有 ToolGroup＋タイル配列）。
    P4.4 で複数タイル対応。`setLayout(1x1/2x1/2x2)`・`active()`・`showSeriesInActive`・`setMeasureTool`（group）・
    `clearAnnotations`/`deleteSelected`。ローカル File は `dicomImageLoader.wadouri.fileManager.add(file)` で
    `dicomfile:` imageId 化（**カスタムローダ不要**）。ツール = W/L(左)/Pan(中)/Zoom(右)/スタック送り(ホイール)。
  - `frontend/portable/src/panel.ts` … `ViewportPanel`（1 タイル）。P4.1/P4.2 の per-viewport ロジック
    （showSeries・W/L(`setWL/getWL/defaultWL`)・トランスフォーム・`setImageIndex/imageIndex/imageTotal`・
    `savePng`＝オーバレイ焼き込み・`onChange`）を保持。オーバレイ更新イベントは自 panel 要素に閉じる。
...
- **P4.4 複数シリーズ／レイアウト／サムネイル** ✅（2026-07-23 実装・検証済）
  - `viewer.ts` を単一 StackViewport ラッパから**マネージャ**へリファクタ（共有 RenderingEngine＋共有
    ToolGroup）。1 タイル分を `panel.ts`（`ViewportPanel`）へ分離（viewport＋4隅オーバレイ／スケールバー＋
    W/L・トランスフォーム・スライス送り・PNG を per-viewport で保持。イベントは各 panel の要素に閉じる）。
  - レイアウト 1×1 / 2×1 / 2×2（`LAYOUTS`。CSS grid `--rows`/`--cols`）。タイルクリックでアクティブ切替
    （枠ハイライト）。ツールバー操作は `viewer.active()` へ委譲、計測は共有 ToolGroup＝ドラッグしたタイルに効く。
    レイアウト変更時は既存タイルのシリーズ割当を可能な範囲で復元（`setLayout` は restore 完了まで await）。
  - シリーズサムネイル（サイドバー・先頭スライスを `csUtilities.loadImageToCanvas({thumbnail,imageAspect})` で
    描画→44px 枠へアスペクト維持で収め直す）。シリーズクリックでアクティブタイルへ表示。
  - 検証: `?selfTest` に P4.4 項目（`panelsAfter2x2`=4・`thumbnails`）。合成操作で 2×2→4タイル・別タイルへ
    2 シリーズ同時表示（tilesWithContent=2）・アクティブ枠1・サムネ非空(21%)・44px。http:// と file:// で ok。
- **P4.5 付帯**（価値小・任意）
