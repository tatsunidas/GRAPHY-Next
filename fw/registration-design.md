...
- 🟡 **実機目視はほぼ完了**（2026-08-08。PSMA whole-body PET/CT で確認）:
  ...
  5. 🔴 **未テスト（TODO）**: IOP/IPP の無いシリーズ（CR/DX 等）で「位置調整」が無効化され、
     理由がツールチップに出ること。**CR/DX を索引に入れるところから**必要なので後回しにした。
     `onSpatialChange` 経由の `adjustEnabled` が false になる経路（`Viewer2DScreen.tsx`）を見る
...
| 第 2 段 | **DICOM SRO**（66.1 / 66.3）＝ R5 | 🔴 未着手 |
