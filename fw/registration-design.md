...
差し替え中に保存が走ると、**記録が新しい（ずれた）指紋で静かに上書きされる**。
保存は `refsRef.current`＝現在のデータの指紋を使うためで、再計算でも手動調整でも起きる。
つまり **指紋照合は読み出し側だけが慎重で、書き込み側は無防備**である。

派生して、「それでも適用」を押しても**その判断がどこにも残らない**という問題もある
（`Viewer2DScreen.tsx` の該当ボタンは表示状態を変えるだけ）。記録の指紋は古いままなので、
開き直すたびに同じ問いを繰り返される。→ §12.4 に対応方針を書く。
...
#### 🔴 残件 — 保存側が指紋に無防備（2026-08-09 の実機検証で判明）

読み出し側は指紋が合わなければ適用しないが、**保存側は現在の指紋で無条件に上書きする**。
再計算でも手動調整でも起きる（どちらも `persistRegistration` を通り、
`refsRef.current`＝**いま表示しているデータ**の指紋で書く）。結果として、
入力が入れ替わったことに気付かないまま「記録は正しい」状態が作られてしまう。

さらに「それでも適用」を押しても、**その判断が記録に残らない**。指紋は古いままなので
開き直すたびに同じ問いを繰り返される。判断が消えるので、後から
「利用者が食い違いを承知で通した」という事実も追えない。

**方針**: 「それでも適用」を押した時点で、**現在の指紋で記録を保存し直す**。
併せて「**指紋不一致のまま承認した**」という印を記録に残す。黙って指紋だけ
書き換えると、後から見て「最初から合っていた」ようにしか見えず、監査として嘘になる。
