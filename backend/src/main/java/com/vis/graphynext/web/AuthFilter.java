 ...
 *
 * <p>{@code /login}・{@code /auth/**} 以外の全リクエストで {@code graphy_session} Cookie の署名・
 * 有効期限を検証する。未ログインの場合、ページ遷移（HTML想定）は {@code /login} へリダイレクト、
 * {@code /api/**}（フロントのXHR/fetch想定）は 401 を返す。
 */
@Component
