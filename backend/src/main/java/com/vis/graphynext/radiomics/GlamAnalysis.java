...
/**
 * GLAM 解析の結果。記述子そのものと、それを潰した特徴量の両方を返す。
 *
 * <p>特徴量 150 個は行列を要約した「答え」だが、解釈に効くのは<b>その手前</b>にある
 * 動径分布関数と親和性行列そのもの（原著論文の図が見せているのはこちら）。数値をそのまま返し、
 * 描画はフロントに任せる。段の関係は {@code fw/texture-radiomics-design.md} §12.9。
 *
 * @param featureCount    この設定で得られる特徴の数（150）
 ...
 * @param crossAffinity   異なる濃度値どうしの親和性 g(α,β,r)。{@code [α][β][r]}（対角も含む完全な形）。
 *                        nBins² × maxRadius が大きすぎる設定では <b>null</b>（{@code crossAffinityOmitted}）
 * @param crossAffinityOmitted 上を返さなかった場合の理由（返した場合は null）
 * @param matrices        親和性行列（19 種）。キーは {@code GLAMMatrixType} 名、値は nBins×nBins
 * @param diagonalOnly    自己ペアだけで定義される行列の名前（非対角は意味を持たない）
 * @param features        特徴量 150 個（{@code GLAMFeatureType} 名 → 値）。行列を統計で潰した「答え」。
 *                        ROI にその濃度値が無い等で定義されない特徴は <b>null</b> が入る
 * @param voxelSpacing    <b>計算に使った</b>ボクセル間隔 (x,y,z) mm。等方でなければ距離の意味が歪む
 * @param isotropic       等方ボクセルか
 * @param resampledFrom   リサンプリングした場合の<b>元の</b>ボクセル間隔 (x,y,z) mm。していなければ null
...
public record GlamAnalysis(
        ...
        double[][][] crossAffinity,
        String crossAffinityOmitted,
        Map<String, double[][]> matrices,
        List<String> diagonalOnly,
        Map<String, Double> features,
        double[] voxelSpacing,
        boolean isotropic,
        double[] resampledFrom,
