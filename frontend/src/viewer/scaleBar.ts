...
/**
 * FOV（＝現在のズーム）に応じて長さを自動調整するスケールバーを求める。
 *
 * <p>画面水平方向に span(px) 離れた 2 点を `canvasToWorld` で world 化し、
 * 1 canvas px あたりの world 量を得る（zoom/回転に追従）。world は
 * <b>校正あり=mm</b>、<b>校正なし=画像ピクセル(spacing=1)</b>。
 * 目標 ~120px に収まる「きりのよい」長さを選び、そのバー長・ラベルを返す。
 */
export function computeScaleBar(
  ...
  try {
    ...
    // バー長は要素幅の ~30%（最大 140px）に収め、常に中央より左に収める。
    // → 下辺中央の向きマーカー(A/P 等)と重ならない。niceNumber は target 以下なので lengthPx ≤ targetPx。
