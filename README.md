...
- **Art of Imaging** — 表示中の画像をパブリックドメインの画家の作風へ翻案する（別リポジトリのプラグイン・**0.3.0 以降**）。下記参照。
...
## Art of Imaging（プラグイン）

表示中の DICOM 画像を、パブリックドメインの画家の作風による作品へ翻案し、鑑賞のための
説明を添えて出力します。**本体には同梱されていません**——別リポジトリのプラグインです。

- リポジトリ: <https://github.com/tatsunidas/graphy-next-plugin-art>
- ギャラリー（投稿先）: <https://vis-ionary.com/art-of-graphy/>

> 研究・教育・芸術表現のためのものです。**診断機器ではありません。**
> 生成された画像を診断・臨床判断に用いないでください。

### 必要なもの

- **GRAPHY-Next 0.3.0 以降。** 外部 AI への送信（`host.ai`）・名前を付けて保存
  （`host.file`）・画面に見えている範囲（`getViewState().visibleRegion`）はいずれも
  0.3.0 で入るホスト API です。満たさない版では**導入時に弾かれます**。
- **Gemini の API キー。** 環境設定 ＞ **外部 AI** で設定します。キーは OS のキーチェーンへ
  預けられ、プラグインからは読めません（`GET /api/settings` にも含まれません）。

### 使い方

1. プラグインマネージャから上記リポジトリの URL で導入し、本体を開き直す
2. 環境設定 ＞ **外部 AI** で API キーを設定する
3. 2D ビューアで画像を開き、**解析 ＞ Art of Imaging**
4. 画家を選んで生成する。**ビューアで拡大・パン・回転・W/L を変えると、送る画像もそれに
   追従します**（画面で構図を決めてから生成できます）

### 🔴 外部送信について

このプラグインは**患者由来の画素を第三者（Google）のクラウドへ送ります。**

- 送信のたびに、**実際に送る画像とプロンプト全文**を示す同意ダイアログを本体が出します。
  内容を確認してから許可してください。同意はセッション内・同一シリーズにのみ効きます。
- `ai-egress` 権限を宣言していないプラグインは、同意を求める前に拒否されます
  （**実際に強制される数少ない権限のひとつ**です）。
- 送信画像に DICOM タグは含まれませんが、**画素に焼き込まれた患者情報は消えません**。
  必要なら 匿名化 ＞ **ピクセル焼き込み除去** で消してから使ってください。
- 無料枠では入力が Google のモデル改善に利用され得ます。患者由来の画像は有料ティアで
  扱ってください。
- 院内規程・倫理審査・患者同意の範囲内でご判断ください。

設計の詳細は [`fw/art-of-imaging-design.md`](fw/art-of-imaging-design.md)、
外部送信の設計判断は [`fw/security.md`](fw/security.md) にあります。
