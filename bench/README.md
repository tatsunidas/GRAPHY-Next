...
## 🚨 圧縮 DICOM を `store-scu.sh` で入れるときの罠（2026-09-21 に踏んだ）

`bench/store-scu.sh`（`StoreScu.java`）は当初 **`ExplicitVRLittleEndian` しか Presentation Context に
提案せず、どのファイルもその構文で送っていた**。GNBP のファントムは全部非圧縮なので長く問題に
ならなかったが、**圧縮ファイル（JPEG Baseline の実データ XA など）を入れると黙って壊れる**。

- 送信側が「非圧縮」と名乗る → 受信側（`DicomStoreScp`）は**ネゴシエートされた構文を正直に記録**する
- 画素はカプセル化された JPEG のまま → **保管された DICOM は「非圧縮」と書いてあるのに中身は JPEG**
- どのビューワで開いても画素として読めない。GRAPHY では**真っ黒**になる

🔴 **エラーも警告も一切出ない。** 取り込みは「ok」と表示され、タグも正しく見え、
画素値の統計（min/max/mean）すら**もっともらしい値になる**——圧縮バイト列はほぼ一様乱数なので
平均が 127 付近に出るためである。**「値が妥当だから読めている」と読み違えやすい。**

✅ **修正済み**: ファイルごとに File Meta の `TransferSyntaxUID` を読み、**その構文を
Presentation Context の先頭で提案し、その構文で送る**。受け入れられなければ dcm4che が例外を
投げるので、**黙って別の構文を名乗ることは無い**。送信ログにも `[JPEG Baseline (Process 1)]` の
ように構文を出すようにした。

⚠️ **この罠で壊れた既存データは、入れ直さないと直らない。** 保管された実体そのものが
書き換わっているため、アプリ側をいくら直しても表示できない。

> 切り分けに使える道具: `frontend/src/viewer/debugApi.ts` の `getImagePixelRange().pixels`。
> `length` と `expected`(= rows×columns) を並べて出す。**この 2 つが食い違っていたら、
> 画素値が妥当に見えても中身は画素ではない。**
...
**患者の対応が分かるデータは患者単位でまとめる。** `patient_pattern` を指定すると、
同じ患者の複数ランが **1 人・1 スタディの下に並び**（シリーズ番号は 1 から連番）、
シリーズだけが分かれる。実物のカテ検査に近い形になる。指定が無いデータは
1 シリーズ = 1 患者のまま（対応が分からないものを勝手にまとめない）。

実データでの確認:
- **DIAS** … 120 シリーズ / 762 フレーム（入力の投影像とちょうど一致。マスク 120 枚は除外）
- **CADICA** … `p41` の 3 ラン（v2 / v8 / v18）が 1 患者・1 スタディ・シリーズ 1〜3 に並ぶ。
  `input/` 配下のゼロ埋め連番なのでディレクトリ単位の束ねで足りた
