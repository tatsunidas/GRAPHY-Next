...
```bash
python3 make_phantom_xa.py --out ./phantom        # 21MB / 7 ファイル
cd ../automator && npx tsx src/spike/xaPhantomCheck.ts
```
...
GNBP-XA-3（2D→3D 再構成）は **A6 が未実装なので作っていない**。使う側が無いデータを
増やしても検証されないため。
...
- 🔴 **径の絶対値が一貫して 13% 過小**（真値 3.000mm → 2.609mm）。実装ミスではなく、
