...
async function main(): Promise<void> {
  ...
  try {
    ...
    check(
      idle.qva?.enabled === "0" && idle.qva?.reason === "xa.task.reason.notImplemented",
    ...
    check(
      (idle.qva?.text ?? "").includes("A5a") && (idle.qca3dBifurcation?.text ?? "").includes("A6b"),
      "[一覧] 未実装カードは担当フェーズを名指しする",
      { qva: idle.qva?.text, bif: idle.qca3dBifurcation?.text },
    );
    ...
    for (const id of ["qca", "qca3d", "qlv", "report"]) {
      check(xa[id]?.enabled === "1", `[XA] ${id} が押せる`, xa[id]);
    }
    check(xa.qva?.enabled === "0", "[XA] 未実装タスクは XA を選んでも押せないまま", xa.qva);
