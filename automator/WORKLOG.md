...
## 2026-09-03 — FFR 骨組みプラグインで本番経路を通す／🚨 **同じ失敗を 3 回した**

`ffrPluginCheck.ts` ＋ `automator/plugins/ffr-skeleton`。A7 の「外部モジュールとの結合は未実施」を
実機 18/0 で埋めた（正本 `fw/angio-design.md` §11.5）。

### 🚨 今日 3 回踏んだ「操作したつもりで何も起きていない」

この repo の JSDoc に**繰り返し書いてあること**を、書いた本人が 3 回踏んだ。形はいずれも同じで、
**「操作を出した」ことと「効いた」ことを別々に確かめていない**。

| # | 何をしたつもりか | 実際 | どう気づいたか |
| :- | :- | :- | :- |
| 1 | TIMI: 画像の上でホイールを回した | ダイアログは画面中央に出るので、**キャンバスの中心はパネルの上**だった。「外で回した」検査も「中で回した」検査も**同じ場所**を回しており、後者が通っていたのも同じ理由 | 「外でも中でもフレームが動かない」→ **製品の不具合と読み違えかけた** |
| 2 | FFR: 3D QCA のダイアログを閉じた | 閉じるボタンに **testid が無く**、`getByRole` の照合が外れていた。**閉じたことを検査していない**ので、次の操作が「ダイアログに塞がれた」という分かりにくい形で落ちた | 次の操作のタイムアウト |
| 3 | FFR: スパイクを直した | **編集が 1 文字も適用されていなかった。** 背景実行にしたため Python の `FileNotFoundError`（cwd 違い）を見落とし、**古いファイルのまま 3 回走らせた** | 出力に新しい検査行が出ないことに気づいて grep |

> 🔑 **運用ルール（今日足したもの）**
> 1. **編集は前景で行い、適用されたことを確認してから**長時間実行に入る。
>    背景実行はエラーを飲み込む——`python3` の `assert` も `FileNotFoundError` も見えない。
> 2. **「押した」の次に「効いた」を検査する。** ダイアログを閉じたら**閉じたことを確かめる**。
>    ホイールを回したら**フレームが動いたことを確かめる**。
> 3. **画面の要素を座標で指すときは、重なりを計算してから指す**
>    （`pointOutsideDialog` のように「相手の外にある点」を実際に求める）。

### 併せて足したもの

- `Xa3dQcaDialog` の閉じるボタンに `data-testid="xa3d-close"`（他の解析ダイアログには全部あった）
- `automator/plugins/ffr-skeleton` — **H11/H12 の本番経路を通すためだけ**の骨組み。
  計算は当てずっぽうで、FFR ではないことを画面にもコードにも書いてある
